...
> **Date :** 2026-05-25
> **Snapshot :** 2026-05-25T21:00:02 UTC
> **Type :** `_update.md` (post-pipeline soir)
> **Analyste :** Desk Argus-IA
> **Réf. précédent :** `AXA_2026-05-25_update.md` (snapshot 13h00 UTC)

---

## Résumé des changements depuis le snapshot précédent

| Élément | État 2026-05-25 13h00 | État 2026-05-25 21h00 | Changement |
|---------|----------------------|----------------------|------------|
...
| Earnings FMP | J0 (2026-05-25) | **J0 (2026-05-25)** | **Confirmé, non résolu** |

**Verdict :** 11e snapshot consécutif sans mutation des données AXA. Le symbole "AXA" reste non reconnu par yfinance (instrument non coté US). L'analyse technique et fondamentale est impossible. Les métriques sectorielles XLF sont inchangées entre 13h00 et 21h00 UTC (return 20j +1.01%, RS 20j −3.43%, momentum 0.0/10). Le marché US était fermé aujourd'hui (Memorial Day), ce qui explique la stabilité totale du snapshot.
...
- RS 20j vs SPY : −3.43% (stable vs 13h00)
- RS 60j vs SPY : −9.03% (stable vs 13h00)
- Momentum score : 0.0/10 (stable)
- Rang sectoriel : 5e/11 (ni top 3 ni bottom 3)

**Interprétation :** Le secteur financier est stable entre les deux snapshots du jour. Sans données AXA, on ne peut évaluer si le titre sur/sous-performe son secteur. Le headwind sectoriel persiste : SPY surperforme XLF de +3.4pp sur 20j. Si les données AXA étaient disponibles, ce contexte peserait sur le score Momentum. Le marché fermé aujourd'hui n'a pas généré de nouvelle information de prix.
...
| Signal | État | Détail |
|--------|------|--------|
| News du jour (`news_2026-05-25.json`) | **Aucune** | `AXA: []` — 0 article |
| Sentiment retail (Reddit) | **No data** | 0 mentions, score 0/10 (`social_sentiment_2026-05-25.json`) |
| Pump / dump detection | 🟢 Aucun | `pump_detected: false` |
| Événements corporate | 🟢 Aucun | `events_2026-05-25.json` → 0 événement AXA |
| Options (max pain, GEX, IV Rank) | **[DONNÉES MANQUANTES]** | Non récupérées |
| Upgrades / downgrades | **[DONNÉES MANQUANTES]** | Non récupérés |

**FX Exposure** (`fx_exposure_2026-05-25.json`) :
...
**Social Sentiment** (`social_sentiment_2026-05-25.json`) :
- AXA mention count : 0
...
| **Évolution** | **Non évaluable** (données de prix absentes + headwind sectoriel stable + marché fermé Memorial Day) |
...
2. **Earnings J0 non résolu :** L'événement earnings du 2026-05-25 est répertorié dans le calendrier FMP mais sans données de consensus ni résultats. L'impact sur le cours ne peut être mesuré. Le passage à J+1 n'apportera pas d'information sans correction du symbole.
...
4. **Marché fermé :** Aucune session de trading US aujourd'hui (Memorial Day). La stabilité totale des métriques entre 13h00 et 21h00 est attendue.
...
*Rapport généré automatiquement par le desk Argus-IA. Données sources : `data/latest.json` (fetched_at 2026-05-25T21:00:02 UTC), `data/recommandations_latest.json`, `data/fx_exposure_2026-05-25.json`, `data/upcoming_events_2026-05-25.json`, `data/social_sentiment_2026-05-25.json`, `data/events_2026-05-25.json`, `data/sector_rotation_2026-05-25.json`, `data/geo_risk_latest.json` (2026-05-17), `data/quant_report_latest.json` (2026-05-17).*
