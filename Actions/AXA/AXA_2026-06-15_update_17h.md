...
> **Date :** 2026-06-15
> **Snapshot :** 2026-06-15T17:00:01 UTC
> **Type :** `_update.md`
> **Analyste :** Desk Argus-IA
> **Réf. précédente :** `AXA_2026-06-15_update.md` (snapshot 10h00 UTC)

---

## Résumé des changements depuis l'analyse précédente

| Élément | État 15/06 10h | État 15/06 17h | Changement |
|---------|---------------|----------------|------------|
...
| Sector rotation XLF (rang JSON 17h) | — | **3e/11** | 🔴 **Divergence vs snapshot 10h (2e/11)** |
| Sector rotation XLF (momentum JSON 17h) | — | **5.12/10** | 🔴 **Divergence vs snapshot 10h (6.73/10)** |
| Sector rotation XLF (RS 20j JSON 17h) | — | **+3.13%** | 🔴 **Divergence vs snapshot 10h (+4.85%)** |
| Signal macro | `UNKNOWN` | **`UNKNOWN`** | **Stable** |
| Earnings FMP | J0 (2026-06-15) sans détails | **J0 (2026-06-15) sans détails** | **Stable** |

**Verdict :** Données AXA toujours indisponibles. **Divergence détectée sur le fichier sectoriel** : le `data/sector_rotation_2026-06-15.json` lu à 17h00 UTC affiche des métriques XLF inférieures à celles citées dans le snapshot 10h (rang 3e vs 2e, momentum 5.12 vs 6.73, RS 20j +3.13% vs +4.85%). Cette divergence n'affecte pas le scoring propre d'AXA (placeholder), mais elle modère l'interprétation du vent de queue sectoriel.

---

## Mise à jour technique

**[DONNÉES MANQUANTES]** Aucun cours, volume, RSI, ATR ou moyenne mobile disponible pour AXA dans `data/latest.json` (snapshot 2026-06-15T17:00:01 UTC). `AXA` est listé dans `tickers_ko` avec raison `"No price history"`.

**Contexte sectoriel XLF — divergence de données :**
- Le fichier `data/sector_rotation_2026-06-15.json` (lu à 17h) est valide mais présente des écarts avec les chiffres rapportés à 10h :
  - XLF : return 20j **+5.37%**, return 60j **+10.48%**
  - RS 20j vs SPY : **+3.13%** (vs +4.85% cité à 10h)
  - RS 60j vs SPY : **−4.38%** (vs −2.97% cité à 10h)
  - Momentum score : **5.12/10** (vs 6.73/10 cité à 10h)
  - Rang sectoriel : **3e/11** (vs 2e/11 cité à 10h)
  - Crossover : `null`
  - Signal macro : **`UNKNOWN`**
- **Interprétation :** les chiffres du JSON 17h suggèrent un contexte sectoriel moins favorable que celui rapporté à 10h. L'écart pourrait provenir d'une correction de données intra-journalière ou d'une dégradation en séance. En l'absence de données propres à AXA, ce contexte sectoriel reste le seul élément observable.
...
- **Impact sur la thèse :** impossible à évaluer. 14e occurrence consécutive sans résolution.
...
**Note sectorielle (ajustée 17h) :** Le fichier JSON `data/sector_rotation_2026-06-15.json` lu à 17h place XLF au **3e/11** (momentum 5.12/10, RS 20j +3.13%), ce qui est moins favorable que les chiffres cités à 10h (2e/11, momentum 6.73/10, RS 20j +4.85%). Cette divergence modère l'interprétation du vent de queue sectoriel mais ne modifie pas le scoring individuel du ticker en l'absence de données propres.
...
### 🟡 Thèse ATTENDRE confirmée — DONNÉES AXA TOUJOURS MANQUANTES, CONTEXTE SECTORIEL EN LÉGERE DIVERGENCE
...
**Divergence sectorielle détectée.** Le fichier `data/sector_rotation_2026-06-15.json` lu à 17h affiche des métriques XLF inférieures à celles du snapshot 10h : rang 3e/11 (vs 2e/11), momentum 5.12/10 (vs 6.73/10), RS 20j +3.13% (vs +4.85%). Cette divergence modère l'interprétation du vent de queue sectoriel sans invalider la thèse.
...
*Desk Argus-IA — Snapshot 2026-06-15T17:00:01 UTC*
