# MITK — Mitek Systems — Mise à jour 2026-06-23 Snapshot 10h UTC

> **Desk :** Argus-IA | Ticker : MITK (NASDAQ) | Secteur : Technology / Software — Application
> **Données :** `data/latest.json` (2026-06-23T10:00Z), `data/recommandations_latest.json`, `data/sector_rotation_latest.json`, `data/fx_exposure_latest.json`, `data/social_sentiment_latest.json`, `data/upcoming_events_latest.json`
> **Analyse précédente :** [MITK_2026-06-22_update_21h.md](MITK_2026-06-22_update_21h.md) — Score Global Ajusté 56.5/100 (ATTENDRE)

---

## 1. Résumé Exécutif & Changements Clés

| Indicateur | Close Officiel 22/06 21h UTC | Snapshot 23/06 10h UTC | Δ |
|------------|------------------------------|------------------------|---|
| **Cours close** | $17.31 | **$17.31** | **$0.00 (0.00%)** |
| **Change % vs previous close** | +0.29% | **+0.29%** | inchangé |
| **Volume** | 987,044 | **1,138,000** | **+15.3%** |
| **Volume vs moy. 20j** | 0.92× | **1.06×** | **+0.14×** |
...
**Verdict :** La thèse est **confirmée — ATTENDRE maintenu**. Stabilité mécanique totale vs close officiel 22/06 : cours, RSI, ATR, MM50 et Forward P/E sont strictement identiques. Seule évolution notable : le volume remonte de **0.92× à 1.06× moyenne 20j** (+15.3%), dépassant la moyenne pour la première fois depuis le snapshot 10h du 22/06. Le cours stable à $17.31 sur volume légèrement supérieur à la moyenne suggère une **absorption au niveau actuel** plutôt qu'une détérioration. Le **Score Global Ajusté reste à 56.5/100**, au centre de la fourchette ATTENDRE (50–59), à 3.5 pts du seuil ACHETER (Réduit).

> **⚠️ Traitement DRAFT_refresh :** Un fichier `MITK_2026-06-23_DRAFT_refresh.md` a été détecté ce matin, déclenché par un trigger `ATR_SPIKE` (5.37%, seuil 5.0%). Après vérification des données brutes, cet ATR_SPIKE est classé comme **faux positif algorithmique (9e occurrence)** — l'ATR absolu ($0.93), le cours ($17.31), le RSI (45.17) et la MM50 ($15.35) sont strictement identiques au close 21h UTC 22/06. Aucune mutation technique n'est observée. Le DRAFT_refresh est archivé.
...
**Verdict timing : Favorable.** La stabilité du cours à $17.31 sur volume en légère surperformance (1.06×) est un signal technique positif : le marché absorbe les flux sans céder de terrain. Le RSI 45.17 reste dans la zone neutre favorable, juste au-dessus de la médiane. Le high $17.90 a été testé puis rejeté, confirmant la résistance sous le 52W high. Le cours reste **+12.8% au-dessus de la MM50** — tendance haussière de MT intacte.
...
| Assets / Liabilities > 1.0 | ✅ | Current ratio FMP 1.19 (révisé vs anciennes divergences) |
...
> **Note divergence Yahoo/FMP :** Le market cap FMP ($446.6M) diverge toujours significativement du market cap Yahoo ($781.7M). Les ratios FMP (current ratio 1.19, interest coverage 1.72x, debt/equity 0.65) sont désormais cohérents avec les valeurs historiques utilisées précédemment. Les métriques Yahoo (market cap, P/E, EV) restent utilisées comme primaires pour le scoring.
...
| **Max pain (Yahoo JSON)** | **$7.50** | Yahoo Finance | 🔴 **Anomalie JSON récurrente** — valeur aberrante |
| **Put/Call ratio (Yahoo JSON)** | **null** | Yahoo Finance | Données corrompues |
| **Call OI % (Yahoo JSON)** | **0.0%** | Yahoo Finance | Données corrompues |
| Expiration options la plus proche | **2026-07-17** (24j) | Yahoo Finance | Inchangée |

> **⚠️ Anomalie options JSON persistante :** Le JSON Yahoo retourne `max_pain: 7.50`, `put_call_ratio: null`, `call_oi_pct: 0.0` — valeurs aberrantes identiques à celles du 2026-06-22 10h. L'analyse du 22/06 13h a validé des valeurs opérationnelles cohérentes : **max pain $17.50**, **put/call 0.05**, **call OI 95.2%**. Ces valeurs opérationnelles sont conservées pour l'évaluation du sentiment. L'écart entre max pain opérationnel ($17.50) et spot ($17.31) est de 1.1% — alignement classique.

**Verdict Sentiment : Neutre à légèrement haussier.** La structure options opérationnelle reste extrêmement haussière (put/call 0.05, call OI 95.2%). Le max pain opérationnel $17.50 est à 1.1% du close. Consensus PT $16.00 toujours dépassé. Aucune mention Reddit. Pas d'insider trades significatifs détectés. Aucune news majeure du jour.
...
**Verdict Macro : Neutre.** Le secteur Technology reste en tête du momentum sectoriel (XLK score 10.0, +7.23% vs SPY sur 20j), vent de queue passif pour MITK. Aucun risque géo, FX ou social détecté.
...
- 2026-06-22 21h : 56.5/100 (ATTENDRE, C:4.0 V:5.0 M:7.0)
- 2026-06-23 10h : **56.5/100** (ATTENDRE, C:4.0 V:5.0 M:7.0)

**Explication :** Aucun changement de score. Les données brutes (cours, RSI, ATR, MM, fondamentaux) sont strictement identiques au close officiel 21h UTC 22/06. Le volume légèrement supérieur à la moyenne (1.06×) n'est pas suffisant pour upgrader le Score Momentum (déjà à 7.0/10) ou le Score Catalyseur (aucune news structurante).
...
**Thèse : CONFIRMÉE — ATTENDRE maintenu, stabilité mécanique totale.**

Le snapshot 10h UTC 2026-06-23 confirme l'intégralité de la lecture du close officiel 21h UTC 22/06. Cours ($17.31), RSI (45.17), ATR ($0.93), MM50 ($15.35) et Forward P/E (14.26x) sont **strictement identiques**. Seule évolution : le volume remonte de 987K (0.92×) à **1,138K (1.06×)**, dépassant la moyenne 20j pour la première fois depuis le snapshot 10h du 22/06. Cette légère surperformance de volume sur cours stable suggère une **absorption au niveau actuel** — signe technique positif mais non déterminant.
...
- Expiration options : **2026-07-17** (24j) — surveillance structure options (max pain opérationnel $17.50 proche du spot)
- Prochain earnings Q3 FY2026 : **2026-08-06** (44j) — Est EPS $0.24–$0.34, Rev ~$0.1B

---

*Généré par Argus-IA — Sources exclusives : data/latest.json, data/recommandations_latest.json, data/sector_rotation_latest.json, data/fx_exposure_latest.json, data/geo_risk_latest.json, data/upcoming_events_latest.json, data/social_sentiment_latest.json — Date : 2026-06-23*
