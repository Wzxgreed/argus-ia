# ASTS — Mise à Jour 2026-06-23 (Snapshot 13h UTC)

> **Close $73,19** : **stabilité mécanique totale** vs snapshot 10h UTC (close officielle 2026-06-22, snapshot 21h UTC). Cours, RSI, ATR, MM50 et volume inchangés au tick près. **Anomalie options JSON résolue** dans `data/latest.json` (snapshot 13h UTC) : max pain $100,00 cohérent, put/call 0,69, call OI 59,3%. Score global ajusté **43,0/100 (SURVEILLER)** inchangé selon JSON recommandations. Thèse **SURVEILLER confirmée** — aucune mutation technique nouvelle depuis le snapshot 10h UTC.

---

## Résumé des Changements depuis l'Analyse Précédente (Snapshot 10h UTC 23/06)

| Indicateur | Snapshot 10h UTC 23/06 | Snapshot 13h UTC 23/06 | Delta |
|-----------|------------------------|------------------------|-------|
| **Cours** | **$73,19** | **$73,19** | **—** 🟢 |
...
| **Volume rel.** | **0,991×** (28,28M) | **0,991×** (28,28M) | **—** 🟢 |
| Volume absolu | 28,28M | **28,28M** | **—** 🟢 |
...
| **Options Max Pain** | **$100,00 (op.)** | **$100,00 (JSON)** | **Résolu** 🟢 |
| **Options Put/Call** | **0,70 (op.)** | **0,69 (JSON)** | **−0,01** 🟢 |
| **Options Call OI %** | **58,9% (op.)** | **59,3% (JSON)** | **+0,4 pt** 🟢 |
...
**Verdict :** le snapshot 13h UTC du 23/06 reproduit mécaniquement le snapshot 10h UTC. Aucun nouveau prix, volume ou indicateur technique n'est disponible. La seule évolution est la **résolution de l'anomalie options JSON récurrente** : les valeurs JSON passent de max pain $45,00 aberrant / put/call null / call OI null à max pain $100,00, put/call 0,69 et call OI 59,3%, cohérents avec les valeurs opérationnelles calibrées manuellement. Cette résolution confirme la fiabilité des valeurs opérationnelles utilisées dans les analyses précédentes.

---

## Anomalies de Données

1. **Anomalie options JSON résolue** dans `data/latest.json` (snapshot 13h UTC) :
   - Max pain JSON : **$100,00** (vs $45,00 aberrant au snapshot 10h) — **résolu**
   - Put/Call ratio JSON : **0,69** (vs null au snapshot 10h) — **résolu**
   - Call OI % JSON : **59,3%** (vs null au snapshot 10h) — **résolu**
   - Cette anomalie était récurrente pour ASTS (9e occurrence depuis le 10/06). La résolution au snapshot 13h UTC valide les valeurs opérationnelles calibrées manuellement ($100,00 / 0,70 / 58,9%).
...
- **Cours :** $73,19 — inchangé vs snapshot 10h UTC. **−9,26%** vs previous close $80,66. **Cumul session précédente : −14,30%** depuis le vrai previous close $85,43
...
- **MM200 :** N/A
- **Volume :** 28,28M vs moy. 20j 28,53M (**0,991×**) — **volume normalisé**, inchangé. Lecture : distribution active confirmée, liquidité au niveau moyen mais direction négative
...
- **Résistances clés :** $80,66 (previous close 13h 22/06) ; $85,43 (vrai previous close 22/06) ; $88,05 (MM50) ; $92,06 (close 09/06) ; $100,00 (psychologique + max pain)
...
Aucun nouveau résultat comptable ni guidance. Données fondamentales inchangées vs snapshot 10h UTC.
...
- **Options (JSON snapshot 13h — anomalie résolue) :**
  - **Max Pain :** **$100,00** — cohérent avec valeur opérationnelle historique. Cours $73,19 reste **−26,8% sous le Max Pain**
  - **Put/Call Ratio :** **0,69** — positionnement défensif/baissier maintenu (vs 0,70 valeur opérationnelle, écart négligeable)
  - **Call OI % :** **59,3%** — skew call stable (vs 58,9% valeur opérationnelle, écart négligeable)
  - Nearest expiry : **2026-06-26** (J+3)
  - **Lecture :** le positionnement options n'a pas changé depuis le snapshot 10h. Les valeurs JSON résolues confirment les valeurs opérationnelles calibrées. Les calls OTM $80+ restent profondément OTM. Le max pain $100 est à +36,6% du spot — pinning gamma très improbable
...
- **Expiration options 2026-06-26 (J+3)** — max pain $100 au-dessus du spot (+36,6%). Pinning gamma hautement improbable. Put/call 0,69 reflète un positionnement défensif stable

---

## Scoring Global — Snapshot 2026-06-23 (13h UTC)

| Axe | Score | Pondération | Commentaire |
|-----|-------|-------------|-------------|
| Catalyseur | 6,5/10 | 35% | Aucun catalyseur imminent, earnings dans 48j. Structure options historiquement haussière (call OI 59,3%) mais stable. Put/call 0,69 = couverture baissière maintenue |
...
- Malus **OPTIONS_BEARISH** : put/call 0,69, call OI 59,3% — positionnement options défensif maintenu, max pain $100 à +36,6%
...
**Révision :** SL et TP **inchangés** vs le snapshot 10h UTC (SL $52,45 / TP $104,30) car le cours et l'ATR sont identiques.
...
- **Alerte options J+3 :** Max Pain $100 à +36,6%. Put/call 0,69 = couverture baissière structurée. Pinning gamma théoriquement possible mais très improbable sous $80
...
**Thèse SURVEILLER confirmée : aucune mutation technique ou fondamentale nouvelle entre le snapshot 10h UTC et le snapshot 13h UTC du 2026-06-23. Score global ajusté 43,0/100 (SURVEILLER).**

Le snapshot 13h UTC du 23/06 apporte une **stabilité mécanique totale** vs le snapshot 10h UTC. L'unique évolution est la **résolution de l'anomalie options JSON récurrente** : les valeurs JSON (max pain $100,00, put/call 0,69, call OI 59,3%) sont désormais cohérentes avec les valeurs opérationnelles calibrées manuellement. Cette résolution valide la fiabilité des valeurs opérationnelles utilisées dans les analyses précédentes.

**Configuration technique inchangée vs le snapshot 10h UTC :**
...
7. **Options résolues** : max pain $100 à +36,6% du spot. Put/call 0,69 stable. Call OI 59,3% — skew call inchangé
...
- **Options J+3** — Max Pain $100 à +36,6%. Put/call 0,69 = couverture baissière structurée
- **Short Interest élevé** — 18,39% — pas de squeeze setup mais pression vendeuse présente

**Verdict opérationnel :** la configuration reste **extrêmement fragile** mais inchangée vs le snapshot 10h UTC. Le niveau $72,59 est toujours le **pivot absolu**.
...
*Généré par le système Argus-IA — Snapshot 2026-06-23, 13h UTC (cours $73,19, RSI 32,85, volume 0,991×, score global 43,0 SURVEILLER, divergence consensus +29,1%, cours −16,8% sous MM50 $88,05, triple gap down total −14,30% du 22/06, low $72,59, options : max pain $100,00, put/call 0,69, call OI 59,3%)*
