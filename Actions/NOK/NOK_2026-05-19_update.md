# NOK — Mise à Jour Quotidienne (2026-05-19 10:00 UTC)

> Desk : Argus-IA | Ticker : NOK (NYSE ADR) | Secteur : Communication Equipment / 5G Infrastructure
> Date analyse : 2026-05-19 | Données source : `data/latest.json` (snapshot 2026-05-19T10:00 UTC)

---

## 1. Résumé des Changements depuis l'Analyse Précédente (2026-05-18 22:34 UTC)

| Indicateur | Update 18/05 22:34 UTC | Snapshot 19/05 10:00 UTC | Variation | Signal |
|-----------|------------------------|--------------------------|-----------|--------|
...
| Volume relatif | 0.70× | **0.73×** | +0.03× | Liquidité inchangée, reste faible |
...
| P/B (Yahoo) | 3.093 | **3.124** | +0.031 | Stable — ajustement calcul marginal |
| EV/EBITDA (Yahoo) | 29.80 | **29.338** | −0.462 | Stable — ajustement calcul marginal |
| Dividend yield | 1.17 % | **1.19 %** | +0.02 pp | Rendement inchangé à l'arrondi |
...
| Put/Call ratio | 0.34 | **null** | — | [DONNÉES PARTIELLES] |
| Max pain options | $15.00 | **2.0** | — | [DONNÉES PARTIELLES — valeur aberrante] |

**Événement majeur du snapshot 19/05 :** aucun événement corporate détecté (`events_2026-05-19.json` vide pour NOK). Aucune news structurante, aucun mouvement d'options ou d'insiders. Le cours de clôture est confirmé à **$13.74** (repli de **−1.51 %** vs la clôture précédente à $13.95).

**Trigger technique :** `ATR_SPIKE` (medium) — ATR relatif 7.06 % (seuil 5.0 %). Évalué comme un **faux positif technique récurrent** : le cours n'a pas bougé d'un cent vs le snapshot 18/05 22:34 UTC, le RSI et l'ATR sont strictement identiques. Le DRAFT_refresh généré par ce trigger est archivé comme non matérialisé (voir `REFRESH_LOG.md`).
...
|----------|--------|-------------|
| Cours close | $13.74 | −1.51 % vs clôture précédente ($13.95) ; stable vs snapshot 18/05 22:34 UTC |
...
| Volume | 91,496,300 | **0.73× moyenne 20j (124,986,445)** — liquidité faible, pas d'accélération |
...
- Max pain options : $15.00 (expiration 2026-05-22) — référence historique NYSE ; donnée du jour aberrante (2.0) [DONNÉES PARTIELLES]

**Verdict timing :** **Favorable** (cours > MM50) mais le volume réduit (0.73×) et l'absence de catalyseur acheteur limitent la conviction.
...
|----------|--------|----------|
| Market Cap | $76.7 B | Stable vs snapshot 18/05 |
| P/E (TTM) | 85.875 | 🔴 Extrêmement élevé |
| Forward P/E | 28.32 | Élevé mais reflète attente de normalisation EPS |
| EV/EBITDA | 29.338 | Premium sectoriel (ajustement calcul −0.462 vs 18/05) |
| P/B | 3.124 | Premium vs book (ajustement calcul +0.031 vs 18/05) |
| Dividend yield | 1.19 % | Support de rendement |

**Données opérationnelles FMP (FY 2025) :** inchangées vs snapshot 18/05. Ratios opérationnels : gross margin 43.5 %, operating margin 3.93 %, net margin 3.27 %, ROE indisponible, ROIC 1.89 % (fmp_key_metrics), net cash (D/E 0.25, current ratio 1.58). Aucune donnée nouvelle ne modifie ce profil.
...
| Put/Call ratio | null | Yahoo Finance — [DONNÉES PARTIELLES] |
| Max pain | 2.0 | Yahoo Finance — valeur aberrante vs historique NYSE $15.00 ; donnée ignorée |
| Short interest | 0.012 % | Yahoo Finance — quasi nulle |
| Agent Social Sentiment | 0 mention, 0.0/10 | `social_sentiment_2026-05-19.json` — aucun buzz retail |
| Agent Event-Driven | Aucun événement | `events_2026-05-19.json` vide pour NOK |

**Upcoming events :**
- Earnings Q2 FY2026 confirmé au **2026-07-23** (dans **65 jours**)
...
Niveaux inchangés vs snapshot 18/05 22:34 UTC. Le SL à $11.80 reste valide.
...
| **Agent Sector Rotation** | XLC bottom 3 | Headwind sectoriel marginal. RS 20j/60j négatif vs SPY. Momentum score 0.0. |
...
Le snapshot 2026-05-19T10:00 UTC confirme intégralement les lectures du snapshot 2026-05-18T22:34 UTC. Le cours de clôture est stable à **$13.74** (repli −1.51 % vs clôture précédente $13.95) sur un volume inchangé et réduit (0.73× moyenne 20j). Le RSI (64.63) et l'ATR ($0.97) sont strictement identiques. Aucun événement corporate, aucune news structurante, aucun mouvement d'options ou d'insiders n'est survenu entre les deux snapshots.
...
Le trigger `ATR_SPIKE` (7.06 %) demeure un **faux positif technique récurrent** : aucun gap, aucune rupture de support, aucun catalyseur. Le DRAFT_refresh généré à 10:00 UTC est archivé comme non matérialisé. La volatilité relative reflète uniquement le range intraday ($13.50–$14.48) sur une liquidité réduite.
