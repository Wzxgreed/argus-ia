# NOK — Mise à Jour Quotidienne (2026-06-03, Snapshot 10:00 UTC)

> Desk : Argus-IA | Ticker : NOK (NYSE ADR) | Secteur : Technology / Communication Equipment
> Date analyse : 2026-06-03 | Données source : `data/latest.json` (snapshot 2026-06-03T10:00:09 UTC)

---

## 1. Résumé des changements depuis l'analyse précédente (Snapshot 21:00 UTC 02/06)

| Indicateur | Snapshot 21:00 UTC (02/06) | Snapshot 10:00 UTC (03/06) | Variation | Signal |
|-----------|---------------------------|---------------------------|-----------|--------|
| Cours close | $16.85 | **$16.85** | — | → Stable |
| RSI 14j | 70.22 | **70.22** | — | → 🔴 Zone surachat confirmée (≥70) |
| ATR 14j | $1.02 | **$1.02** | — | → Stable |
| Volume | 129,557,620 | **134,743,500** | **+4.0%** | → 🔵 Légère révision à la hausse |
| Volume vs moy. 20j | 1.09× | **1.13×** | +0.04× | → Participation toujours au-dessus de la moyenne |
...
- **→ 🔵 Volume légèrement révisé à la hausse (+4.0%)** : de 129.6M (1.09× moyenne 20j) à 134.7M (**1.13× moyenne 20j**). Cette micro-révision confirme que la participation au rally reste légèrement supérieure à la moyenne 20j, ce qui valide la solidité technique du new 52w high.
- **→ Cours strictement inchangé** à $16.85. Aucune mutation overnight.
- **→ RSI 70.22** : surachat confirmé et persistant. Aucun recul technique en pre-market.
- **→ Tous les scores agents inchangés** (`recommandations_latest.json` non muté).
- **→ Aucune news, aucun événement corporate** : `news_latest.json` et `events_latest.json` vides pour NOK.
- **→ Anomalie options détectée** dans `latest.json` (max pain $2.00, put/call null, call OI null) — données corrompues à ignorer ; valeurs opérationnelles du 02/06 conservées ($13.50 / 0.45 / 69.1%).
...
|----------|--------|--------|-------------|
| Cours close | $16.85 | Yahoo Finance | Inchangé vs snapshot 21h 02/06, +3.69% vs previous close ($16.25) |
...
**Niveaux clés (inchangés vs 21h 02/06) :**
...
**Mise à jour options — données opérationnelles (02/06 conservées, anomalie 03/06 ignorée) :**
| Niveau | Valeur opérationnelle (02/06) | Valeur `latest.json` (03/06) | Interprétation |
|--------|------------------------------|------------------------------|----------------|
| Max pain | $13.50 | **$2.00** (anomalie) | Corrompu — valeur 02/06 conservée |
| Put/Call ratio | 0.45 | **null** (anomalie) | Corrompu — valeur 02/06 conservée |
| Call OI % | 69.1% | **null** (anomalie) | Corrompu — valeur 02/06 conservée |
| Expiration | 2026-06-05 | **2026-06-05** | ✅ Confirmée — dans **2 jours** |

> **🔴 Pin risk extrème maintenu à l'expiration 05/06 (dans 2 jours).** Le cours ($16.85) reste **+24.8% au-dessus du max pain** ($13.50). La pression de mean-reversion vers $13.50 persiste. Avec **2 jours restants** avant expiration vendredi, et en l'absence totale de catalyseur, la probabilité d'un ajustement baissier technique reste élevée. La micro-révision du volume à 1.13× renforce légèrement la solidité du niveau, mais ne supprime pas la surchauffe technique (RSI 70) ni l'absence de justification fondamentale.
...
**Note fondamentale :** Aucune donnée fondamentale nouvelle depuis le snapshot 21h 02/06. Le consensus inchangé à $9.26 sur 6 analystes maintient la divergence à **+81.9%**. Aucun upgrade, downgrade ou révision d'estimations n'a été détecté.
...
| Put/Call ratio | 0.45 | Snapshot 02/06 (conservé) | Structure bullish (dominance calls) — anomalie 03/06 ignorée |
| Max pain | $13.50 | Snapshot 02/06 (conservé) | 🔴 Risque pin baissier extrème à l'expiration dans 2 jours |
| Call OI % | 69.1% | Snapshot 02/06 (conservé) | Forte activité call — stable |
...
| Axe | Score (JSON 10h 03/06) | Évolution (vs 21h 02/06) | Justification |
|-----|-----------------------|-------------------------|---------------|
...
| Niveau | Ancien (21:00 UTC 02/06) | Nouveau (10:00 UTC 03/06) | Justification |
|--------|--------------------------|--------------------------|---------------|
...
**⚠️ Attention :** Le volume légèrement révisé à la hausse (1.13× moyenne 20j) confirme que le rally vers $17.11 a été accompagné par une participation supérieure à la moyenne, ce qui renforce légèrement la solidité technique du new high. Cependant, le **pin risk options ($13.50, expiration dans 2 jours)** reste la menace principale : le cours est +24.8% au-dessus du max pain. Sans catalyseur, la probabilité d'un ajustement baissier technique avant ou à l'expiration reste élevée. Si le cours casse sous $16.25 (close 01/06) en clôture demain, le rejet du new high sera confirmé.
...
**Verdict :** La thèse est **confirmée** — le snapshot 10:00 UTC du 03/06 confirme intégralement la configuration du snapshot 21:00 UTC du 02/06, avec une micro-nuance positive sur le volume.

**Analyse :**
- **Technique :** Cours strictement inchangé $16.85. Le **volume est légèrement révisé à la hausse de 4.0%** : 134.7M (1.13× moyenne 20j) vs 129.6M (1.09×) au snapshot 21h 02/06. Cette micro-révision confirme que la participation au rally reste légèrement supérieure à la moyenne 20j, ce qui valide la solidité technique du new 52w high. Cependant, le **RSI reste à 70.22** (zone surachat) et le cours reste +44.0% au-dessus de la MM50 ($11.70) — surchauffe persistante.
- **Options :** Données **corrompues dans `latest.json`** (max pain $2.00, put/call null, call OI null) — anomalie identifiée et ignorée. Valeurs opérationnelles du 02/06 conservées (max pain $13.50, put/call 0.45, call OI 69.1%). Le pin risk reste extrème : cours +24.8% au-dessus du max pain, expiration **dans 2 jours** (05/06).
...
**Ce qui a changé (21:00 UTC 02/06 → 10:00 UTC 03/06) :**
- **Volume :** 129.6M (1.09×) → **134.7M (1.13×)** — micro-révision à la hausse, confirmant la participation au-dessus de la moyenne
- **Aucun autre changement** — cours, RSI, ATR, P/E, scores, consensus, événements : tous inchangés

**Ce qui n'a pas changé :**
- **Options :** max pain $13.50 (opérationnel), put/call 0.45, call OI 69.1%, expiration 05/06 dans 2 jours
...
*Données sources : `data/latest.json` (2026-06-03T10:00:09 UTC), `data/recommandations_latest.json`, `data/quant_report_latest.json`, `data/geo_risk_latest.json`, `data/sector_rotation_latest.json`, `data/social_sentiment_latest.json`, `data/fx_exposure_latest.json`, `data/upcoming_events_latest.json`, `data/events_latest.json`. Valeurs options corrompues dans `latest.json` (max pain $2.00, put/call/call OI null) — valeurs opérationnelles du snapshot 02/06 conservées. Aucune donnée hallucinée.*
