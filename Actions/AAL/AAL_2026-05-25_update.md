# AAL — Mise à Jour 2026-05-25

**Date :** 2026-05-25 (snapshot 13:00 UTC)  
**Ticker :** AAL (NASDAQ)  
**Type :** Update post-session — Données fraîches vs clôture 2026-05-20  
**Cours (close) :** $13.85  
**Previous close :** $13.59  
**Change session :** +1.91%  
**Volume :** 99.26M (vs moy. 20j 57.88M, +71.5%)

---

## Résumé des Changements depuis l'Update (2026-05-20)

| Indicateur | 2026-05-20 | 2026-05-25 | Δ vs Prior |
|-----------|------------|------------|------------|
| Cours close | $12.06 | **$13.85** | **+$1.79 (+14.8%)** |
| RSI 14j | 60.62 | **71.43** | **+10.81 pts** |
| ATR 14j | $0.47 | **$0.55** | **+$0.08 (+17.0%)** |
| MM 50j | $11.56 | **$11.71** | **+$0.15 (+1.3%)** |
| Forward P/E | 5.45 | **6.26** | **+0.81 (+14.9%)** |
| Volume du jour | 67.4M vs 51.9M avg (+29.8%) | **99.26M vs 57.88M avg (+71.5%)** | **+41.7 pts** |
| Short Interest | 12.21% | **12.21%** | **0.00** |
| Consensus FMP PT | $16.02 (15 analystes) | **$16.02 (15 analystes, 0 màj mois)** | **Inchangé** |
| Upside vs PT | +32.8% | **+15.7%** | **−17.1 pts** |
| Put/Call Ratio | 2.67 | **1.82** | **−0.85 (−31.8%)** |
| Max Pain | $9.50 | **$13.00** | **+$3.50 (+36.8%)** |
| Call OI % | 27.2% | **35.5%** | **+8.3 pts** |
| Score Opportunité agent | 6.6/10 | **5.7/10** | **−0.9 pt** |
| Recommandation agent | ACHETER (Sizing Réduit) | **SURVEILLER** | **Downgrade** |

**Session active.** Le snapshot du 2026-05-25 reflète une nouvelle session de marché avec gap haussier et poursuite du rally entamé après le 20/05. Le cours a gagné +14.8% en 5 séances, franchissant l'ancien take-profit ($13.47) et testant la zone de résistance $14.00–$14.18 (gap janvier 2026). Le volume a explosé à 99.26M (+71.5% vs moyenne), confirmant un flux institutionnel ou un short-covering massif. Les données options sont complètes et cohérentes (pas d'anomalie data quality vs le snapshot du 20/05).

---

## Mise à Jour Technique

| Indicateur | Valeur | Signal |
|-----------|--------|--------|
| Cours | $13.85 | +1.91% session ; +14.8% vs close 20/05 |
| RSI 14j | 71.43 | 🔴 **Surachat** — franchissement zone 70 |
| ATR 14j | $0.55 | Volatilité en expansion (+17% vs 20/05) |
| MM 50j | $11.71 | 🟢 Cours +18.3% au-dessus (trend haussier intact) |
| MM 200j | null | [DONNÉES MANQUANTES] |
| Volume 20j | 57.88M | 🟡 **+71.5% vs moyenne** — flux anormal, probable short-covering |
| 52W Range | $10.09–$16.50 | Cours à 84% du 52W low, 16.1% sous le 52W high |
| Support clé | $13.50 | Ancienne résistance / gap janvier → support à valider |
| Support secondaire | $12.75 | Cours − 2×ATR = niveau technique de sortie |
| Résistance | $14.00–$14.18 | Gap janvier 2026 + high du jour |
| Résistance majeure | $16.50 | 52W high |
| Short Interest | 12.21% | 🟡 Élevé — fuel squeeze partiellement consommé |

**Options — Données complètes :**

| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| Put/Call Ratio | **1.82** | 🟡 Baissier atténué (−31.8% vs 2.67) mais >1.5 |
| Max Pain | **$13.00** | Réalignement avec le cours — pinning mécanique probable autour de $13.00 à expiration |
| Call OI % | **35.5%** | Repositionnement call confirmé et amplifié (+8.3 pts) |
| Expiration proche | 2026-05-29 | Dans 4 jours — gamma risk concentré autour de $13.00 |

**Interprétation technique :**
- Le rally +14.8% en 5 séances est un mouvement de grande amplitude pour un legacy airline. Il s'explique probablement par un short-covering massif (volume +71.5%, short interest stable à 12.21% = les shorts n'ont pas recouvert en masse, ce qui laisse du fuel).
- **RSI 71.43 = surachat.** Historiquement, AAL corrige dans les 3–5 jours après franchissement de RSI 70. La probabilité de consolidation ou de repli technique est élevée.
- **Max Pain $13.00 vs cours $13.85** : le cours est au-dessus du max pain à 4 jours de l'expiration. Cela est inhabituel et indique un momentum call fort. Le risque de pinning vers $13.00 existe si le momentum faiblit avant le 29/05.
- **Volume 99.26M** : c'est le volume le plus élevé observé sur les 20 derniers jours. Un volume extrême sur rally = distribution possible si le cours ne tient pas au-dessus de $14.00. À surveiller.
- **Niveau critique : $13.50.** Si cassure en clôture sous ce niveau = fin de l'impulsion haussière. Sous $12.75 = sortie technique obligatoire (2×ATR).

---

## Mise à Jour Fondamentale

### Consensus Analystes — Stable mais figé
- **Price Target moyen FMP : $16.02** (15 analystes, **0 mise à jour le mois dernier**, 3 le trimestre dernier)
- **Upside implicite : +15.7%** vs cours $13.85 (vs +32.8% le 20/05)
- **Couverture :** 15 analystes — coverage significatif mais dormante

### Ratios FMP — Bilan Stressé (inchangé)
| Ratio | Valeur | Seuil | Signal |
|-------|--------|-------|--------|
| P/E (LTM, Yahoo) | 44.68 | — | 🔴 Élevé (charges récentes) |
| Forward P/E | **6.26** | — | 🟡 Réévaluation mécanique +14.9% — moins cheap |
| P/B (Yahoo) | -2.25 | — | 🔴 Equity négatif |
| P/B (FMP) | -2.72 | — | 🔴 Equity négatif |
| P/S (FMP) | 0.185 | — | 🟢 Très faible |
| EV/EBITDA (Yahoo) | 8.83 | — | 🟡 Élevé vs industrie |
| EV/EBITDA (FMP) | 11.44 | — | 🟡 Élevé vs Yahoo |
| EV/Revenue (Yahoo) | 0.65 | — | 🟢 Faible |
| EV/Revenue (FMP) | 0.81 | — | 🟢 Faible |
| Gross Margin | 19.2% | — | 🟡 Sector norm |
| Operating Margin | 2.7% | — | 🔴 Faible |
| Net Margin | 0.2% | — | 🔴 Quasi nul |
| Current Ratio | 0.50 | >1.0 | 🔴 Trésorerie insuffisante |
| Quick Ratio | 0.38 | >0.8 | 🔴 Liquidity stress |
| Net Debt / EBITDA | 8.83x | <3.0 | 🔴🔴 Extrême |
| Interest Coverage | 0.85x | >2.0 | 🔴 Service dette > EBIT |
| Tangible Asset Value | -$9.88B | >0 | 🔴 Patrimoine négatif |
| Working Capital | -$12.3B | >0 | 🔴 Capacité opérationnelle négative |
| ROE | -3.0% | >10% | 🔴 Destruction de valeur |
| ROIC | 2.0% | >8% | 🔴 Très faible |
| FCF Yield | -6.7% | >0 | 🔴 FCF négatif |
| Debt/Equity | -9.65 | — | 🔴 Equity négatif |
| Revenue per Share | $82.72 | — | 🟢 Élevé (revenue massive) |
| Book Value per Share | -$5.64 | >0 | 🔴 Equity négatif |

**Forward P/E 6.26** (vs 5.45 le 20/05) — la hausse du cours (+14.8%) a mécaniquement réduit l'asymétrie valorisation. Le forward EPS implicite reste ~$2.21/share. À ce niveau, AAL n'est plus une value trap évidente mais n'a pas de marge de sécurité.

### Événement Clé — Earnings Q2 FY2026
- **Date :** 2026-07-23 (59 jours)
- **Estimates EPS :** -$0.34 à $0.17
- **Estimates Revenue :** $16.6B
- **Implication :** La fourchette EPS large reflète l'incertitude. Un beat au-dessus de $0.17 reste un catalyseur majeur compte tenu du short interest 12.2% et du put/call 1.82. Cependant, le timing de l'entrée à $13.85 est moins favorable qu'à $12.06.

---

## Mise à Jour Sentiment / Options / Flux / Macro

### Sentiment Analystes
- **Figé :** 15 analystes FMP, PT $16.02. Aucune mise à jour le mois dernier. Le consensus institutionnel est en attente d'un catalyseur (earnings 23/07).

### Social Sentiment
- **Reddit / Yahoo Community :** 0 mentions. Aucun pump/dump détecté.
- **Label agent :** EXTREME_BEARISH (valeur 0.0) — absence de buzz = indifférence retail. Pas de signal contrarian ici.

### Options
- **Put/Call 1.82** (vs 2.67 le 20/05) : repositionnement call confirmé mais le sentiment reste nettement baissier (>1.50). La baisse du ratio indique une couverture des shorts options ou un achat de calls spéculatifs.
- **Max Pain $13.00** (vs $9.50) : réalignement complet avec le cours. À expiration 29/05, le pinning mécanique devrait se concentrer autour de $13.00. Le cours à $13.85 est vulnérable à un repli de $0.85 vers le max pain.
- **Call OI 35.5%** : forte accumulation de calls — si le cours tient au-dessus de $13.50 jusqu'à expiration, le gamma squeeze pourrait continuer. Sinon, décharge gamma baissière rapide.

### Exposition Macro
| Facteur | Exposition | Mise à jour |
|---------|-----------|-------------|
| Taux 10Y US | 🔴 Élevée | Inchangée — dette variable, +1% = +$400M/an |
| Pétrole (WTI) | 🔴🔴 Critique | Inchangée — jet fuel 25-30% coûts |
| DXY | 🟡 Modérée | 🟢 FX Exposure Score 0.0 (neutral, pas de headwind/tailwind) |
| Industriels (XLI) | 🔴 Défavorable | **XLI bottom3 sector rotation (momentum 0.0)** — vent de secteur défavorable |

### Sector Rotation
- **Industrials (XLI)** : return 20d −0.41%, RS20 vs SPY −4.85%. **Bottom3** du ranking sectoriel avec momentum score 0.0. Pas de crossover détecté.
- **Impact :** Vent de secteur défavorable. Le rally d'AAL est idiosyncratique (short-covering) et non soutenu par la rotation sectorielle.

### Géopolitique
- **Score Politique :** 2/10 — AAL non exposé aux événements géopolitiques actuels.
- **Pas d'ajustement** sur le score global.

### Accounting Risk / Quant
- **Accounting risk :** Fichier `accounting_risk_latest.json` **indisponible**. Le Filtre Qualité (0-1/6) et les ratios FMP (interest coverage <1x, tangible asset value négative) suggèrent une santé financière très faible. Pas de nouvelle alerte comptable.
- **Quant report :** Données insuffisantes — 0 signaux historiques, calibration en cours. Pas d'alerte de significativité.

---

## Score Opportunité Révisé

| Axe | 2026-05-20 /10 | 2026-05-25 /10 | Δ | Justification |
|-----|----------------|----------------|---|---------------|
| Catalyseur | 6.8 | **6.3** | −0.5 | Consensus PT $16.02 inchangé mais upside réduit à +15.7%. Earnings 23/07 reste le catalyseur clé. Pas de news structurante. |
| Valorisation | 6.5 | **5.5** | −1.0 | Forward P/E 6.26 (+14.9%) = moins cheap. Upside vs PT divisé par deux. Filtre qualité 0-1/6 intact. |
| Momentum | 6.5 | **5.3** | −1.2 | RSI 71.43 = surachat. Rally +14.8% en 5j = probabilité de consolidation élevée. Volume extrême = avertissement distribution. |
| **Score Opportunité** | **6.6** | **5.7** | **−0.9** | Pondération 35/40/25 (régime inconnu = default) |

**Score Global Composite agent :** 57.3/100 → **Ajusté 47.3/100**
- Malus : geo 0, FX 0, event 0, social 0, quant 0
- Timing : **Défavorable**
- **Recommandation agent : SURVEILLER**

**Verdict institutionnel Argus-IA :** La thèse tactique ACHETER sizing réduit du 20/05 est **partiellement matérialisée** (+14.8% en 5 séances). Le trade virtuel aurait atteint et dépassé l'ancien TP ($13.47). À ce stade, le ratio risque/rendement s'est dégradé : le SL à $12.75 implique une perte potentielle de −7.9% pour un upside de +11.9% (TP $15.50). La valorisation n'est plus aussi attractive et le RSI surachat indique un risque de repli technique élevé. Le secteur Industrials est en sous-performance. **Downgrade de ACHETER à SURVEILLER / ATTENDRE une consolidation.**

---

## Niveaux SL / TP Révisés

| | 2026-05-20 | 2026-05-25 | Justification |
|---|------------|------------|---------------|
| Entrée suggérée | $12.06 | **$13.85** | Close actuel — **Ne pas entrer à ce niveau** |
| Stop-Loss | $11.12 | **$12.75** | Cours − 2×ATR = $13.85 − $1.10. Aligné sur support $12.75–$13.00 |
| Take-Profit | $13.47 | **$15.50** | Cours + 3×ATR = $13.85 + $1.65. Objectif technique sous 52W high |
| Ratio R/R | 1.5 | **1.5** | — |

**Note institutionnelle :** Les niveaux ont été révisés mécaniquement à la hausse. Le SL $12.75 correspond à la zone $12.75–$13.00 (support technique post-rally). Une cassure sous $12.75 en clôture = invalidation du trend haussier court terme. Le TP $15.50 est conservateur (ancien gap + résistance psychologique). Si le cours franchit $14.18 (high du jour) sur volume > 80M, le TP peut être révisé à $15.50–$16.00. **Expiration options 29/05 dans 4 jours** : le Max Pain $13.00 vs cours $13.85 indique un risque de repli mécanique de $0.85 si le momentum call faiblit.

---

## Conclusion — Thèse Confirmée, Modifiée ou Invalidée ?

**Verdict : MODIFIÉE — La thèse tactique est partiellement matérialisée. Downgrade de ACHETER à SURVEILLER.**

### Ce qui a changé (snapshot 2026-05-25 13:00 UTC) :
1. **Cours +14.8%** — Rally puissant en 5 séances, atteignant et dépassant l'ancien TP ($13.47). Le trade virtuel du 20/05 aurait généré +11.7% (objectif atteint).
2. **RSI 71.43** — Franchissement de la zone surachat (70). Probabilité de consolidation/repli technique élevée dans les 3–5 jours.
3. **Volume 99.26M (+71.5%)** — Explosion volumétrique. Probable short-covering massif ou flux institutionnel. Avertissement de distribution si le cours ne tient pas $14.00.
4. **Forward P/E 6.26** (+14.9%) — Réévaluation mécanique. L'asymétrie valorisation s'est réduite.
5. **Put/Call 1.82** (−31.8%) — Atténuation du sentiment extrême baissier. Repositionnement call confirmé (Call OI 35.5%).
6. **Max Pain $13.00** (vs $9.50) — Réalignement complet avec le cours. Gamma risk concentré à expiration 29/05 autour de $13.00.
7. **Score agent downgrade 6.6 → 5.7** — Réduction du score opportunité et passage à SURVEILLER.
8. **Sector rotation défavorable** — Industrials (XLI) dans le bottom3 (momentum 0.0). Le rally n'est pas soutenu par le secteur.

### Ce qui n'a PAS changé :
1. **Consensus analyste FMP** : PT $16.02 inchangé (15 analystes, 0 mise à jour mois).
2. **Filtre Qualité : 0-1/6** — Hors périmètre compounding. AAL reste une commodité sans moat, bilan stressé.
3. **Bilan extrêmement fragile** — Current ratio 0.50, interest coverage 0.85x, tangible asset value -$9.88B, working capital -$12.3B.
4. **Short Interest 12.21%** — stable. Le fuel de squeeze n'a pas été consommé en masse (volume = probablement acheteurs longs, pas uniquement short-covering).
5. **Régime macro défavorable** — Stagflation (fuel, taux élevés, salaires) = pire environnement pour les airlines.
6. **Earnings Q2 = binary event** — 59 jours. Estimates EPS large = forte volatilité post-announcement.

### Risques identifiés (révisés)
1. **Surachat technique (RSI 71.43)** — Risque de repli de −5% à −8% vers $12.75–$13.00 dans les prochains jours.
2. **Volume extrême sur rally = distribution possible** — Si la prochaine session affiche volume > 80M avec un cours sous $13.50, signal de distribution institutionnelle.
3. **Gamma risk à expiration 29/05** — Dans 4 jours. Max Pain $13.00 vs cours $13.85 = repli mécanique de $0.85 plausible.
4. **Value trap réactivé** — Forward EPS ~$2.21/share peut ne pas se matérialiser si fuel/grèves/récession. Le forward P/E 6.26 n'est pas une protection.
5. **Accounting risk non quantifié** — Absence de scan comptable (M-Score, Z-Score, F-Score, Sloan).
6. **Vent de secteur défavorable** — XLI sous-performe. Un rally sectoriel est nécessaire pour soutenir une continuation au-dessus de $14.00.

### Positionnement Argus-IA
- **Action : SURVEILLER / ATTENDRE** — Ne pas entrer à $13.85. Le trade tactique a eu lieu entre $12.06 et $13.47.
- **Si position virtuelle ouverte depuis le 20/05 :** Réduire de 50% ou placer un stop trailing à $12.75. Le gain +11.7%–14.8% doit être protégé.
- **Horizon :** 1–3 mois (jusqu'à earnings Q2 + réaction post-announcement) mais l'entrée est désormais défavorable.
- **Catalyseur clé :** Earnings 2026-07-23. Préparer un nouvel entry si repli vers $12.50–$12.75 sur volume normalisé.
- **Si cours < $12.75 (SL) :** Sortie technique complète — trend haussier invalidé.
- **Si cours > $14.18 (high du jour) sur volume > 80M :** Réévaluer le momentum. TP révisable à $15.50–$16.00.
- **Si cours < $13.50 avant expiration 29/05 :** Sortie anticipée — fin de l'impulsion + risque gamma baissier.
- **Si put/call repasse sous 1.50 avec volume calls > 40% OI :** Setup contrarian transformé en setup haussier pur — réévaluer la thèse.

---

## [UNSOURCED]
- MACD, MM200, IV Rank, earnings whisper, insider trades détaillés, 13F complets, ETF flows, dark pool, transcripts NLP, job postings.
- Accounting risk (M-Score, Z-Score, F-Score, Sloan) — fichier `accounting_risk_latest.json` indisponible.
- Données quantitatives significatives (p-value, Sharpe) — insuffisantes.

---

## Références
- `data/2026-05-25.json` (snapshot 13:00 UTC) — Cours $13.85, RSI 71.43, ATR $0.55, MM50 $11.71, volume 99.26M, short interest 12.21%, consensus FMP $16.02, options (put/call 1.82, max pain $13.00, call OI 35.5%), Forward P/E 6.26
- `data/recommandations_2026-05-25.json` — Score Opportunité 5.7/10, Score Global 57.3/100 (ajusté 47.3), Recommandation SURVEILLER, SL $12.75, TP $15.50
- `data/validation_report.txt` (2026-05-25) — 5 errors globales (AST/AXA/CYTOMX/QTBS fetch failed ; VRT schema), 2 warnings (IREN, NOK). AAL non concerné.
- `data/sector_rotation_2026-05-25.json` — XLI bottom3 (momentum 0.0)
- `data/fx_exposure_2026-05-25.json` — FX Impact Score 0.0, neutral
- `data/social_sentiment_2026-05-25.json` — Sentiment retail 0 mentions (EXTREME_BEARISH)
- `data/upcoming_events_2026-05-25.json` — Earnings 2026-07-23, 59 jours
- `data/events_2026-05-25.json` — Aucun événement corporate détecté
- `data/quant_report_latest.json` — Données quantitatives insuffisantes
- `data/geo_risk_2026-05-25.json` — Score Politique 2/10, non exposé
- `Agents/AGENT_FONDAMENTAL.md` — Méthodologie Filtre Qualité
- `Agents/AGENT_TECHNIQUE.md` — Méthodologie technique
- `Agents/AGENT_SENTIMENT.md` — Méthodologie sentiment
