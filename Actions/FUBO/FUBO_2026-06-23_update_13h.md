# FUBO — Mise à jour 2026-06-23 (snapshot 13h UTC)

> **Ticker :** FUBO | **Secteur :** Communication Services / Broadcasting
> **Close :** $8.82 (−4.34% vs previous close $9.22) | **Volume :** 1,139,100 (0.79× moy. 20j 1.44M)
> **Source données :** `data/latest.json` (2026-06-23 13:00:18 UTC) + `data/recommandations_latest.json`

---

## 1. Résumé des changements depuis l’analyse précédente (2026-06-23 10h UTC)

| Indicateur | 2026-06-23 10h UTC | 2026-06-23 13h UTC | Δ |
|------------|-------------------|--------------------|---|
| **Close** | $8.82 | **$8.82** | **Inchangé — stabilité mécanique totale** |
| **Previous Close** | $8.82 (erroné) | **$9.22** | **Corrigé par Yahoo — aligne le change_pct à −4.34%** |
| **RSI 14j** | 27.72 | **27.72** | **Inchangé — survente maintenue** |
| **Volume vs 20j** | 0.79× (1.14M) | **0.79×** (1.14M) | **Inchangé** |
| **MM50** | $10.78 | **$10.78** | **Inchangé** |
| **Écart MM50** | −18.2% | **−18.2%** | **Inchangé** |
| **ATR 14j** | $0.81 | **$0.81** | **Stable** |
| **Max Pain** | $9.00 (opérationnel)* | **$9.00** | **Anomalie JSON 10h résolue** |
| **Put/Call** | 0.66 (opérationnel)* | **0.66** | **Anomalie JSON 10h résolue** |
| **Call OI %** | 60.3% (opérationnel)* | **60.3%** | **Anomalie JSON 10h résolue** |
...
\* *Snapshot 10h UTC : `data/latest.json` retournait max_pain $7.50, put/call `null`, call OI 0.0% — valeurs aberrantes traitées comme anomalie JSON récurrente. Le snapshot 13h UTC corrige ces données avec des valeurs cohérentes ($9.00 / 0.66 / 60.3%).*

> **Événement matériel :** Aucun. Le snapshot 13h UTC confirme la **stabilité mécanique totale** des données brutes de marché (cours, RSI, ATR, MM, volume) par rapport au snapshot 10h UTC. La seule mutation est la **résolution de l’anomalie options JSON** : le JSON 13h retourne désormais des valeurs opérationnelles cohérentes (max pain $9.00, put/call 0.66, call OI 60.3%) qui remplacent l’artefact aberrant ($7.50 / null / 0.0%) du snapshot 10h. Le `previous_close` est corrigé à $9.22, ce qui rétablit le change_pct officiel à **−4.34%** (déjà observé au close 22/06 21h UTC). Le scoring agent n’a pas recalculé de nouveaux scores — les scores du `recommandations_latest.json` restent identiques (57.8/100).

**Verdict :** Données de marché **stables**. La configuration technique (RSI 27.72, écart MM50 −18.2%, spot sous max pain) est inchangée. L’anomalie options est résolue. **Thèse ATTENDRE confirmée** — pas d’entrée en l’état.
...
| **Close** | $8.82 | Identique au snapshot 10h UTC |
...
Aucune nouvelle donnée fondamentale publiée depuis le snapshot 10h UTC. Les métriques restent inchangées :
...
| **P/B (Yahoo)** | 0.320 | Patrimoine net négatif — discount profond |
| **P/B (FMP)** | 3.193 | Divergence Yahoo/FMP sur P/B également |
...
### Options — Anomalie JSON RÉSOLUE (structure pinning neutre / légèrement baissier)

| Indicateur | Valeur 23/06 13h UTC | Signal |
|------------|---------------------|--------|
| **Max Pain** | $9.00 | Spot **−2.0%** au-dessus — pinning légèrement baissier |
...
> **Résolution anomalie :** Le snapshot 10h UTC retournait max_pain **$7.50**, put_call_ratio **null**, call_oi_pct **0.0%** — pattern JSON corrompu. Le snapshot 13h UTC corrige ces valeurs avec des données cohérentes : **$9.00 / 0.66 / 60.3%**. Ces valeurs confirment la structure pinning neutre / légèrement baissière déjà documentée.

> **Implication :** Le spot ($8.82) reste **−2.0% sous le max pain** ($9.00). À J+3 de l’échéance, le spot sous le max pain expose à une pression vendeuse gamma si le cours ne remonte pas au-dessus de $9.00 avant vendredi.
...
- **Aucun changement** vs le snapshot 10h UTC : le scoring agent n’a pas recalculé de nouveaux scores sur un snapshot stable.
...
### Verdict : **THÈSE ATTENDRE CONFIRMÉE — STABILITÉ MÉCANIQUE TOTALE, ANOMALIE OPTIONS RÉSOLUE**

Le snapshot 2026-06-23 13h UTC confirme la **stabilité mécanique totale** des données brutes de marché par rapport au snapshot 10h UTC : cours inchangé ($8.82), RSI 27.72 (survente maintenue), volume stable (1.14M, 0.79×), écart sous MM50 −18.2% inchangé, distance au 52W low maintenue à **$0.51**. Le scoring agent n’a pas produit de nouveaux scores — les scores du `recommandations_latest.json` restent identiques : **Score Global Ajusté 57.8/100**, Momentum **4.0/10**, confirmant que la survente n’est pas interprétée comme un setup positif.

**Mutation principale :** L’**anomalie options JSON récurrente** du snapshot 10h (max_pain $7.50, put/call null, call OI 0.0%) est **résolue** dans le snapshot 13h. Les valeurs corrigées ($9.00 / 0.66 / 60.3%) confirment la structure pinning neutre / légèrement baissière déjà documentée. Le `previous_close` est corrigé à $9.22, rétablissant le change_pct officiel à **−4.34%** (déjà connu du close 22/06 21h UTC).
...
4. **Structure options résolue et stable** : max pain $9.00 cohérent
...
**Recommandation :** **ATTENDRE** — pas d’entrée en l’état. La configuration technique, fondamentale et sectorielle est **strictement inchangée** vs le snapshot 10h UTC. L’anomalie options JSON récurrente (max_pain $7.50 aberrant) a été **résolue** dans le snapshot 13h et les valeurs opérationnelles ($9.00 / 0.66 / 60.3%) sont confirmées. Le DRAFT_refresh déclenché par ATR_SPIKE (9.19%) est traité : ce signal de volatilité persistant est déjà intégré dans la thèse depuis 5 sessions et ne modifie pas la conclusion. Le titre reste un spéculatif fondamental dégradé (Qualité 1/6) sans catalyseur observable, sous exposition gamma vendeuse à J+3. Aucune position longue recommandée. Surveillance renforcée si rupture du 52W low.

---

*Rapport généré par le desk Argus-IA — Données sources : `data/latest.json` (2026-06-23 13:00:18 UTC), `data/recommandations_latest.json`, `data/sector_rotation_latest.json`, `data/upcoming_events_latest.json`, `data/social_sentiment_latest.json`, `data/fx_exposure_latest.json`, `data/events_latest.json`.*
