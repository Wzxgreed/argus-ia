...
> **Niveau d'impact :** 🟡 Modéré — Données techniques inchangées, earnings jour J en attente, malus sectoriel actif
...
**Constat :** Snapshot `data/latest.json` inchangé (timestamp 10:00:12 UTC). Aucun nouveau flux de prix ni de consensus post-séance. L'earnings Q1 2026 reste jour J sans résultat publié à ce stade.
...
> **Règle absolue :** Score ≤ 3/6 → Score Valorisation plafonné à 5/10. L'anomalie de scoring persiste dans `recommandations_latest.json` (Valorisation affichée 7,0/10) ; le malus qualité n'est pas appliqué par le moteur. En ajustement manuel, le score Valorisation effectif ne dépasse pas 5/10.
...
- Crossover détecté : **BULLISH_CROSSOVER** sur XLC (signal contradictoire)

**Impact FUBO :** Malus sectoriel **−0,5 pt** sur le Score Opportunité. Le crossover bullish est un potentiel reversal, non confirmé par le momentum effectif.
...
|---|---|---|
| Max Pain | $21,00 | Loin du spot — pinning peu probable |
| Put/Call Ratio | 0,65 | Skew nettement call-biased |
| Call OI % | 60,6% | Positionnement haussier near-term |

**Lecture :** Repositionnement options vers les calls à J-4 de l'échéance suggère une anticipation de volatilité autour de l'earnings. Le max pain à $21 est si éloigné qu'il perd de sa pertinence pratique. Le call-skew reflète un pari spéculatif sur un beat, pas un consensus haussier structuré.
...
|---|---|---|
| Score Global | 56,8 / 100 | **~52 / 100** |
| Score Opportunité | 6,5 / 10 | **5,2 / 10** |
...
4. **Divergence Yahoo/FMP persistante** — le P/E 2,5x et le market cap $283M sont suspects. Le profil FMP (EV/EBITDA 16x, P/B 3,2x, market cap $3,3B) correspond davantage à une small-cap spéculative à haut risque.
5. **Sector rotation défavorable** — XLC (Communication Services) dans le bottom 3 sectoriel.
...
**Recommandation finale :** **ATTENDRE — pas de position.** Si résultats positifs, le titre reste une spéculation court terme et non un investissement long terme. Le Score Qualité 1/6 et le patrimoine net négatif excluent toute conviction structurelle.
