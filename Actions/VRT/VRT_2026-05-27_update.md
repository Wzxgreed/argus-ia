# VRT — Mise à Jour 2026-05-27 (Snapshot 13:00 UTC)

**Date :** 2026-05-27
**Snapshot :** 13:00 UTC (données pre-market / close 2026-05-26)
**Cours :** $323.91 (−1.08% vs clôture précédente $327.46)
...
## Résumé des changements depuis l'analyse précédente (Snapshot 10:00 UTC 2026-05-27)

| Indicateur | Snapshot 10:00 UTC (27/05) | Snapshot 13:00 UTC (27/05) | Δ |
|---|---|---|---|
...
| Volume | **6.50M** (1.13×) | **6.50M** (1.13×) | ✅ Inchangé |
| Put/Call Ratio | **null** (anomalie) | **2.14** | 🟢 **RÉSOLU** — anomalie corrigée, −0.25 pt vs 26/05 |
| Max Pain | **$145** (anomalie) | **$225.00** | 🟢 **RÉSOLU** — anomalie corrigée, confirmé stable |
| Call OI % | **null** (anomalie) | **31.8%** | 🟢 **RÉSOLU** — anomalie corrigée, +2.3 pts vs 26/05 |
| P/E (Yahoo TTM) | **81.38** | **81.38** | ✅ Inchangé |
| Forward P/E | **36.65** | **36.65** | ✅ Inchangé |
| EV/EBITDA (Yahoo) | **52.53** | **52.53** | ✅ Inchangé |
| EV/Revenue | **11.54** | **11.54** | ✅ Inchangé |
...
| Jours jusqu'earnings | **63** | **63** | ✅ Inchangé |

> **Verdict :** Le snapshot 13:00 UTC du 27 mai confirme une **stabilité totale** des données de cours et techniques vs le snapshot 10:00 UTC. La principale évolution est la **résolution de l'anomalie options** détectée ce matin : `data/latest.json` retourne désormais des valeurs cohérentes (put/call 2.14, max pain $225, call OI 31.8%) vs les artefacts du matin (null / $145 / null). Le put/call 2.14 représente une **légère amélioration** par rapport au 26/05 (2.39) — le sentiment options reste très bearish mais s'atténue marginalement. Le call OI remonte à 31.8% (+2.3 pts vs 29.5%). La thèse reste **SURVEILLER** avec la même nuance baissière — la consolidation $324–$335 reste sous pression, le support $324 n'a pas été retesté en séance ce matin (pre-market). À surveiller : ouverture du marché US ce jour (14:30 UTC) pour confirmer ou infirmer la tenue du support.
...
| Métrique | Valeur 26/05 21:00 | Valeur JSON 27/05 13:00 | Δ | Commentaire |
|---|---|---|---|---|
| Max Pain | **$225.00** | **$225.00** | ✅ 0 | Confirmé stable — anomalie JSON résolue |
| Put/Call Ratio | **2.39** | **2.14** | 🟢 −0.25 | Légère amélioration — reste très bearish (68.2% puts) |
| Call OI % | 29.5% | 31.8% | 🟢 +2.3 pts | Calls remontent marginalement |
...
- Options bearish maintenues (put/call 2.14, 68.2% puts) — risque volatilité à expiration vendredi (2 jours restants)
...
|---|---|---|---|
| P/E (Yahoo TTM) | Yahoo | **81.38** | Stable vs 10:00 UTC. Prime de croissance IA massive |
| Forward P/E | Yahoo | **36.65** | Inchangé. Réflète croissance >30% attendue |
| EV/EBITDA (Yahoo TTM) | Yahoo | **52.53** | Stable vs 10:00 UTC. Premium massif |
| EV/Revenue | Yahoo | **11.54** | Stable |
...
| Put/Call Ratio | **2.14** (résolu JSON) | 🔴 Sentiment options très bearish. 68.2% puts — légèrement moins extrême que 2.39 |
| Max Pain | **$225.00** (résolu JSON) | Gap 30.5% vs cours — confirmé stable |
| Call OI % | 31.8% (résolu JSON) | Calls remontent marginalement (+2.3 pts) |
...
> **Règle absolue :** Score Valorisation ≤ 2.5/10 + catalyseur faible = **SURVEILLER STRICT**. Le score Opportunité reste à **3.9/10** (inchangé vs 10:00 UTC). Verdict **SURVEILLER** maintenu avec nuance baissière.
...
**Verdict : THÈSE CONFIRMÉE — STABILITÉ POST-REJET, ANOMALIE OPTIONS RÉSOLUE, FONDAMENTAUX INTACTS, PRESSION TECHNIQUE BAISSIÈRE MAINTENUE**
...
- **Options :** 🟡 **Légèrement amélioré** — Put/Call 2.14 (vs 2.39 le 26/05). Max pain $225 confirmé. **Anomalie JSON résolue** — valeurs cohérentes désormais retournées. Call OI remonte à 31.8% (+2.3 pts). Risque de volatilité à expiration vendredi inchangé (2 jours restants).
- **Volume :** 🟡 **Stable** — 6.50M (1.13× moyenne). Stable vs hier, toujours au-dessus de la moyenne 20j.
- **Timing :** 🟡 **Neutre → Défavorable** (inchangé) — RSI sous 45, rejet intraday 26/05 non infirmé, options bearish maintenues.
...
- **Données Pipeline :** ✅ Intègres — 23/26 tickers OK, VRT sans erreur [CRITICAL]. Anomalie options du matin résolue.
...
- **Urgence :** Expiration options vendredi 29 mai avec max pain $225 (gap 30.5%) et put/call 2.14 — risque de volatilité élevée. 68.2% puts = potentiel de short squeeze si catalyst inattendu, mais le rejet du 26/05 affaiblit cette probabilité.
...
- 🔴 **Put/Call 2.14** — Sentiment options très bearish. 68.2% puts vs 31.8% calls. Max pain $225 = gap 30.5%
...
- 🔴 **Options expiration 29 mai** — Risque volatilité élevée à 2 jours. 68.2% puts = potentiel squeeze affaibli par rejet
...
- [SNAPSHOT PRE-MARKET] — Le snapshot 13:00 UTC du 27 mai reflète le pre-market US (ouverture 14:30 UTC). Les données de cours ($323.91) sont identiques au close du 26 mai. La session US du 27 mai n'a pas encore eu lieu au moment du snapshot.
- [ANOMALIE OPTIONS RÉSOLUE] — `data/latest.json` retourne désormais des valeurs cohérentes pour VRT : max pain $225.00, put/call 2.14, call OI 31.8%. L'anomalie du matin ($145, null, null) est résolue. L'historique VRT montre des anomalies options récurrentes (18/05 : put/call 3.08, 19/05 : put/call 2.62, 20/05 : anomalie 0.0/$190/100%, 25/05 : put/call 2.59, 26/05 : put/call 2.39, 27/05 10:00 : null/$145/null). La stabilité relative des valeurs autour de 2.14–2.59 vs les artefacts extrêmes confirme la méthode de maintien des valeurs confirmées.
...
*Analyse générée le 2026-05-27 avec snapshot 13:00 UTC. Stabilité confirmée post-rejet intraday du 26/05. Anomalie options JSON résolue. Thèse confirmée avec pression baissière maintenue. Ne pas modifier — créer un nouvel `_update.md` pour toute révision.*
