# FLY — Mise à Jour (2026-06-09, snapshot 13h UTC)

> **Type :** `_update.md` — Snapshot 13h UTC, stabilité confirmée vs 10h UTC, anomalie options RÉSOLUE, thèse SURVEILLER (43.8) inchangée
> **Référence précédente :** [FLY_2026-06-09_update.md](FLY_2026-06-09_update.md) (snapshot 10h UTC 09/06)
> **Données source :** `data/2026-06-09.json` (timestamp 2026-06-09T13:00:08.616483+00:00), `data/recommandations_2026-06-09.json`, `data/sector_rotation_2026-06-09.json`, `data/social_sentiment_2026-06-09.json`, `data/fx_exposure_2026-06-09.json`, `data/upcoming_events_2026-06-09.json`, `data/events_2026-06-09.json`, `data/quality_gate_2026-06-09.json`
> **Validation data :** FLY status `ok` dans `data/validation_report.txt` et `data/quality_gate_2026-06-09.json`. Aucun warning. 25/29 tickers OK.

---

## Résumé — Stabilité confirmée, anomalie options résolue, thèse SURVEILLER (43.8) inchangée

Le snapshot 13h UTC du 2026-06-09 confirme la **stabilité totale** des données vs le snapshot 10h UTC. Le cours reste à **$36.18** (+0.22% vs prior close $36.10), sur un **volume de 4.21M (0.45× moy. 20j)**. Toutes les métriques techniques, fondamentales et de scoring sont strictement identiques.

**Changement significatif vs snapshot 10h UTC :**
- **Options — Anomalie RÉSOLUE** : les données options dans `data/2026-06-09.json` sont désormais cohérentes. Max pain **$40.00** confirmé (vs $19.00 aberrant dans le snapshot 10h). Put/call **1.00** (vs 1.08 opérationnel 10h). Call OI **50.0%** (vs 48.1% opérationnel 10h). Légère détente du ratio put/call (−7.4%) et rééquilibrage des OI vers 50/50.

**Stabilité complète sur tous les autres axes :**
- **Cours** : $36.18 → **$36.18** (inchangé)
- **RSI 14j** : 41.52 → **41.52** (inchangé)
- **ATR 14j** : $5.79 → **$5.79** (inchangé)
- **MM 50j** : $38.81 → **$38.81** (inchangé)
- **Volume** : 4.21M → **4.21M** (inchangé)
- **Scores agents** : inchangés — C 6.0, V 5.5, M 3.5, Opportunité 5.2, Global Ajusté 43.8
...
| ATR 14j | $5.79 | **$5.79** | Inchangé |
| Volume session | 4.21M | **4.21M** | Inchangé |
| Volume vs moy. 20j | 0.45x | **0.45x** | Inchangé |
| Options — Max Pain | $40.00 (op.) / $19.00 (anom.) | **$40.00** | ✅ Anomalie résolue |
| Options — Put/Call | 1.08 (op.) | **1.00** | −7.4% (légère détente) |
| Options — Call OI % | 48.1% (op.) | **50.0%** | +1.9 pt (rééquilibrage) |
| Score Opportunité | 5.2/10 | **5.2/10** | Inchangé |
| Score Momentum | 3.5/10 | **3.5/10** | Inchangé |
| Score Global Ajusté | 43.8 | **43.8** | Inchangé |
| Action | SURVEILLER | **SURVEILLER** | Confirmée |
| Timing | Défavorable | **Défavorable** | Confirmé |

**Verdict :** Le snapshot 13h UTC confirme la **stabilité totale** du cours et des métriques. La **résolution de l'anomalie options** est le seul événement notable : les données brutes Yahoo sont désormais cohérentes (max pain $40.00, put/call 1.00, call OI 50.0%), éliminant le doute data du snapshot 10h. L'Agent Recommandation maintient **SURVEILLER (43.8)**.

---

## Mise à jour technique — Stabilité totale, anomalie options résolue

| Indicateur | Valeur | Verdict |
|------------|--------|---------|
| Cours close | $36.18 | +0.22% vs prior close $36.10, −51.0% vs 52W high $73.80 |
| Open | $37.18 | Gap haussier minime vs prior close — inchangé |
...
| Résistance 2 | $40.00 (Max Pain) | Aimant options confirmé — niveau de référence |
| Résistance 3 | $38.20 (High du jour) | Résistance intraday testée et rejetée |

**Timing verdict :** **Défavorable** — Inchangé. Le cours $36.18 reste sous la MM50 ($38.81) de −6.8%, avec un RSI stable à 41.52 et un Score Momentum à 3.5/10. L'expiration options J-3 (2026-06-12) avec max pain $40.00 et spot $36.18 (−9.6%) positionne le marché options légèrement favorable aux puts. La zone $35.55–$35.70 reste le support critique à surveiller.
...
## Mise à jour sentiment / options / news — Anomalie résolue, silence persistant
...
| Max Pain | **$40.00** | Yahoo Finance 13:00 UTC | ✅ **Anomalie résolue** — valeur confirmée cohérente (vs $19.00 aberrant à 10h). |
| Put/Call Ratio | **1.00** | Yahoo Finance 13:00 UTC | Légère détente vs 1.08 (10h) — rééquilibrage neutre. |
| Call OI % | **50.0%** | Yahoo Finance 13:00 UTC | Rééquilibrage à 50/50 (vs 48.1% à 10h). |
...
**Note sur le scoring :** L'Agent Recommandation maintient FLY en **SURVEILLER (43.8)**. Tous les scores sont strictement identiques au snapshot 10h UTC. Le Score Opportunité (5.2/10) franchit le seuil 5.0 mais le Score Global Ajusté tombe dans la fourchette 35–49 (SURVEILLER).
...
- **Quality Gate** : FLY status `ok` — aucune exclusion.
...
|--------|--------|---------|-------------|
| Cours actuel | $36.18 | Snapshot 13h UTC 09/06 | +0.22% vs prior close $36.10 |
...
**Risque technique persistant :** L'expiration options J-3 (2026-06-12) avec max pain $40.00 et spot $36.18 (−9.6%) pourrait créer une pression mécanique. En dessous de $35.55, le prochain support structuré reste vers $33.00–$34.00 (zone de gap fill du rally de mai). Une cassure de cette zone sur volume > 1.0× moy. 20j ouvrirait le chemin vers les $30.00.

---

## Conclusion — Thèse défavorable confirmée, stabilité sans catalyst, anomalie options résolue — SURVEILLER (43.8)

**Verdict : Thèse défavorable CONFIRMÉE — SURVEILLER (43.8). Le snapshot à $36.18 (+0.22%) sur volume 0.45× n'est pas significatif. Aucun changement matériel depuis le snapshot 10h UTC.**

Le snapshot 13h UTC du 09/06 confirme la **stabilité complète** de la dégradation technique : cours $36.18 (+0.22% vs prior close), volume 4.21M (0.45× moy. 20j), RSI stable à 41.52, Score Momentum stable à 3.5/10. L'**anomalie options est résolue** : max pain confirmé à $40.00 (cohérent), put/call 1.00, call OI 50.0%.
...
- **Divergence Yahoo/FMP** sur Market Cap ($5.94B vs $3.40B) et P/B (5.24 vs 2.86) persistante — [DONNÉES PARTIELLES].

**Ce qui modifie la thèse (aucun) :**
- Aucun changement matériel depuis le snapshot 10h UTC.
- La résolution de l'anomalie options n'impacte pas la thèse (les valeurs opérationnelles étaient déjà utilisées).
...
9. **Pin risk options J-3** : max pain $40.00, spot $36.18 — puts $40.00 in-the-money.
...
*Snapshot 13:00 UTC 09/06 — Cours $36.18 (+0.22% vs prior close $36.10, −51.0% vs 52W high), RSI 41.52 neutre-basse, volume 4.21M (0.45× moy. 20j). Consensus inchangé $43.25 (12 analysts). Options : anomalie RÉSOLUE — max pain $40.00, put/call 1.00, call OI 50.0%, expiration 2026-06-12 (J-3). Aucun catalyst. Fondamentaux inchangés et défavorables. Agent Recommandation : SURVEILLER (43.8). Thèse défavorable confirmée.*
