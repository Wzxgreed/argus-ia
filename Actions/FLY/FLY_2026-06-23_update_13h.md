# FLY — Mise à jour 2026-06-23 (snapshot 13h UTC)
...
| Métrique | Snapshot 13h UTC 23/06 | Snapshot 10h UTC 23/06 | Δ |
|----------|------------------------|------------------------|---|
| Cours close | **$28.96** | **$28.96** | **stable** |
| Volume session | **6.22M** | **6.22M** | **stable** (0.63×) |
...
| **Max pain** | **$50.00** | **$18.00** | **[ANOMALIE DATA RÉSOLUE]** |
| **Put/Call ratio** | **0.36** | **null** | **[ANOMALIE DATA RÉSOLUE]** |
| **Call OI %** | **73.8%** | **null** | **[ANOMALIE DATA RÉSOLUE]** |
...
**Observations clés :**
- **Stabilité totale des données de cours, volume et technique** vs le snapshot 10h UTC du 23/06. Le snapshot 13h UTC confirme le carry-over du close officiel du 22/06 sans nouveau mouvement intraday.
- **[ANOMALIE DATA OPTIONS RÉSOLUE]** : `data/latest.json` snapshot 13h UTC retourne désormais des données options **cohérentes** : max pain **$50.00**, put/call **0.36**, call OI **73.8%**. Ces valeurs valident les données opérationnelles conservées du close 22/06 ($50.00/0.35/74.0%) et invalident l'anomalie du snapshot 10h UTC (max pain $18.00 aberrant, put/call null, call OI null). L'écart put/call (+0.01) et call OI (−0.2 pt) est dans la marge de fluctuation normale inter-snapshot.
- **Fondamentaux inchangés** : Filtre Qualité 2/6, marges négatives, non rentable.
- **Aucun nouveau DRAFT_refresh** déclenché — stabilité mécanique confirmée.
...
**Verdict timing :** Défavorable — aucun changement technique depuis le snapshot 10h UTC. RSI survente extrême persistante sans divergence haussière. Absence totale de pattern de retournement. Le cours est en dessous de la MM50 de −25.7%.

---

## 3. Mise à jour Fondamentale

Aucun nouveau catalyst fondamental. Données strictement identiques au snapshot 10h UTC :

| Métrique | Valeur 13h UTC 23/06 | Valeur 10h UTC 23/06 | Δ |
|----------|----------------------|----------------------|---|
...
| Put/Call ratio | **0.36** | [RÉSOLU] — cohérent vs 0.35 du close 22/06, skew haussier extrême |
| Max pain | **$50.00** | [RÉSOLU] — cohérent vs close 22/06, −42.1% vs spot |
| Call OI % | **73.8%** | [RÉSOLU] — cohérent vs 74.0% du close 22/06, dominance calls |
...
- **[ANOMALIE DATA RÉSOLUE]** : `data/latest.json` snapshot 13h UTC confirme la restauration des données options : **max pain $50.00**, **put/call 0.36**, **call OI 73.8%**. Ces valeurs valident la structure observée au close 22/06 et invalident l'anomalie du snapshot 10h UTC (max pain $18.00 aberrant, put/call null, call OI null).
- **Max pain $50.00** vs spot $28.96 = **−42.1% de distance** à J-3. Le rebond nécessaire pour pin (+72.6%) est impossible sans catalyst majeur.
- **Put/call 0.36** — niveau très faible, proche du 0.35 du close 22/06. Skew haussier extrême persistant.
- **Call OI 73.8%** — dominance calls extrême, quasi-identique au 74.0% du close 22/06. Les acheteurs de calls parient sur un rebond technique malgré la tendance baissière.
- **Verdict :** La structure options est désormais **validée et stable** sur deux snapshots consécutifs (close 22/06 et 13h UTC 23/06). Le signal contradictoire persiste : skew haussier extrême alors que le spot est en chute libre. À J-3, les calls OTM $35–$40 risquent d'expirer sans valeur. La pression vendeuse post-expiration pourrait s'accentuer si les détenteurs de calls baissent leurs positions.
...
Niveaux inchangés vs snapshot 10h UTC — aucun mouvement de cours ni de volatilité.
...
Le snapshot 13h UTC du 23/06 ne présente **aucun changement matériel** par rapport au snapshot 10h UTC. Les données de cours, volume, RSI, ATR, MM50 et fondamentaux sont strictement identiques.
...
**Ce qui a changé :**
- **[ANOMALIE DATA RÉSOLUE]** Options corrompues du snapshot 10h UTC (max pain $18.00 aberrant, put/call null, call OI null) **validées et corrigées** dans le snapshot 13h UTC : max pain **$50.00**, put/call **0.36**, call OI **73.8%** — cohérents avec le close 22/06.
- **DRAFT_refresh du 23/06 archivé** — faux trigger du snapshot 10h UTC (carry-over close 22/06) non réactivé.
