# IREN — Mise à Jour (2026-06-16, snapshot 13:00 UTC)

> **Type :** `_update.md` — Révision post-pipeline (snapshot 13h UTC)
> **Référence précédente :** [IREN_2026-06-16_update.md](IREN_2026-06-16_update.md) (snapshot 10h UTC)
> **Données source :** `data/latest.json` (fetched_at 2026-06-16T13:00:08 UTC), `data/recommandations_latest.json`, `data/quant_report_latest.json`, `data/geo_risk_latest.json`, `data/sector_rotation_latest.json`, `data/social_sentiment_latest.json`, `data/fx_exposure_latest.json`, `data/upcoming_events_latest.json`, `data/events_latest.json`
> **Trigger :** DRAFT_refresh 2026-06-16 13h — traitement + archivage automatique

---

## Résumé des Changements (vs Snapshot 10h UTC 2026-06-16)

| Métrique | 10h UTC 16/06 | 13h UTC 16/06 | Δ |
|----------|---------------|---------------|---|
| **Cours close** | **$60.85** | **$60.85** | **=** |
| **Previous close** | $59.77 | **$59.77** | **=** |
| **Change % session** | **+1.81%** | **+1.81%** | **=** |
| **Volume** | 33.08 M | **33.08 M** | **=** (différence d'arrondi) |
| **Volume vs 20j** | 67.4% | **67.4%** | **=** |
| **RSI 14j** | 51.08 | **51.08** | **=** |
| **ATR 14j** | $6.18 | **$6.18** | **=** |
| **MM 50j** | $52.58 | **$52.58** | **=** |
...
| **EV/EBITDA (Yahoo)** | 159.66× | **159.66×** | **=** |
| **EV/Revenue (Yahoo)** | 31.04× | **31.04×** | **=** |
| **P/B** | 7.79× | **7.79×** | **=** |
| **Max Pain** | $100.00 (anomalie) | **$35.00** | **⚠️ CORRECTION** |
| **Put/Call ratio** | null (anomalie) | **1.44** | **⚠️ CORRECTION** |
| **Call OI %** | 0.0% (anomalie) | **41.0%** | **⚠️ CORRECTION** |
...
**Verdict global : STABILITÉ DES DONNÉES BRUTES, CORRECTION OPTIONS MAJEURE.**

Le snapshot 13h UTC du 2026-06-16 confirme la stabilité absolue du cours ($60.85), des volumes (33.08 M), des indicateurs techniques (RSI 51.08, ATR $6.18, MM50 $52.58) et du scoring algorithmique (Score Global ajusté **57.5/100**, action **ATTENDRE**). L'événement structurel du snapshot est la **résolution de l'anomalie options** détectée à 10h UTC : les données brutes Yahoo retournent désormais une structure cohérente et plausible (Max Pain $35.00, put/call 1.44, call OI 41.0%), substituant la structure aberrante du matin (Max Pain $100.00, put/call null, call OI 0.0%).

**Structure options 13h UTC vs référence 21h UTC 15/06 :**
- Max Pain : **$35.00** (vs $40.00 le 15/06) — **↓ −$5.00** — niveau plus bas, aligné avec la dynamique de défiance
- Put/Call : **1.44** (vs 1.62 le 15/06) — **↓ −0.18** — défiance légèrement atténuée mais persistante
- Call OI : **41.0%** (vs 38.1% le 15/06) — **↑ +2.9 pts** — léger rééquilibrage vers les calls

**Interprétation institutionnelle :** La structure options corrigée confirme une défiance du marché maintenue (put/call 1.44 > 1.0, puts dominants 59.0% OI), mais atténuée par rapport au pic de défiance du 15/06 (put/call 1.62). Le Max Pain à $35.00 (vs close $60.85) indique que les writers d'options profitent d'un cours éloigné du strike médian — ce qui est typique d'une volatilité élevée et d'une dispersion des strikes. L'expiration 2026-06-18 est dans **2 jours**.
...
|------------|--------|-------------|
| **RSI 14j** | 51.08 | Zone neutre, inchangé. Favorable |
...
| **Open / High / Low** | $62.32 / $63.17 / $60.34 | Range intraday 4.6% — inchangé |
...
| Signal | Valeur 13h UTC | Évolution vs 21h UTC 15/06 | Commentaire |
|--------|---------------|---------------------------|-------------|
| **Consensus PT (FMP)** | **$69.12 (26 analysts)** | = | Inchangé |
| **Max Pain** | **$35.00** (exp 2026-06-18) | ↓ −$5.00 | **Nouvelle structure valide** — plus bas que référence $40.00 |
| **Put/Call ratio** | **1.44** | ↓ −0.18 | **Nouvelle structure valide** — défiance atténuée mais persistante |
| **Call OI %** | **41.0%** | ↑ +2.9 pts | **Nouvelle structure valide** — rééquilibrage calls |
...
La correction de l'anomalie options du snapshot 13h UTC rétablit une structure cohérente. Comparée à la référence du 15/06 :
- Le Max Pain à **$35.00** est significativement sous le close ($60.85), ce qui conforte la lecture d'une volatilité anormale et d'une dispersion des strikes étendue. Les writers d'options sont en position favorable.
- Le put/call **1.44** (vs 1.62 le 15/06) indique une légère détente de la défiance, mais les puts restent majoritaires (59.0% OI). C'est un signal de prudence institutionnelle persistante.
- Le call OI à **41.0%** est légèrement supérieur à la référence 38.1%, ce qui pourrait refléter des achats de protection call-spread ou des paris haussiers contrariants sur le gamma de l'expiration 18/06.
...
## Scoring Global (Agent Recommandation — 2026-06-16, snapshot 13h UTC)
...
> 3. **Multiples extrêmes** — P/E 79.0×, EV/EBITDA ~160×, Forward P/E −64.7×. Toute hausse est purement spéculative/momentum.
> 4. **Anomalie options résolue** — La structure du snapshot 13h UTC (Max Pain $35.00, put/call 1.44, call OI 41.0%) est cohérente et devient la nouvelle référence opérationnelle. Expiration 18/06 dans 2 jours.
...
**Thèse : CONFIRMÉE — statut ATTENDRE inchangé sur stabilité totale des données brutes, structure options corrigée.**

Le snapshot 13h UTC du 2026-06-16 confirme la **stabilité absolue** des données brutes de cours, volumes et indicateurs techniques par rapport au snapshot 10h UTC. L'événement structurel est la **correction de l'anomalie options** : les données Yahoo retournent désormais une structure plausible (Max Pain $35.00, put/call 1.44, call OI 41.0%) qui devient la référence opérationnelle, substituant les valeurs aberrantes du matin (Max Pain $100.00, put/call null, call OI 0.0%).

**Différentiels clés vs snapshot 10h UTC 16/06 :**
1. **Cours** : $60.85 → $60.85 (=)
2. **Volume** : 33.08 M → 33.08 M (=)
3. **RSI / ATR / MM50** : inchangés
4. **Multiples** : inchangés
5. **Scores** : Opportunité 5.3→5.3, Global 57.5→57.5 — **=**
6. **Action** : ATTENDRE → **ATTENDRE** (=)
7. **Options** : Max Pain $100.00 (anomalie) → **$35.00** (correction valide) ; put/call null → **1.44** ; call OI 0.0% → **41.0%**. **Nouvelle référence opérationnelle établie.**
8. **SL/TP** : $48.49/$79.39 (R/R 1.5) — inchangés

**Différentiels clés vs référence 21h UTC 15/06 :**
- Max Pain : $40.00 → **$35.00** (↓ −$5.00) — niveau plus bas, volatilité anormale confirmée
- Put/Call : 1.62 → **1.44** (↓ −0.18) — défiance légèrement atténuée
- Call OI : 38.1% → **41.0%** (↑ +2.9 pts) — rééquilibrage marginal vers les calls
...
- **Attention expiration 18/06** : le Max Pain fiable à $35.00 reste un niveau bas éloigné du cours ($60.85). Risque de volatilité anormale si le cours rejette $63.17 sans volume. Les writers d'options sont favorisés.
...
*Rapport rédigé le 2026-06-16 — Données sources : `data/latest.json` (fetched_at 2026-06-16T13:00:08 UTC), `data/recommandations_latest.json`, `data/quant_report_latest.json`, `data/geo_risk_latest.json`, `data/sector_rotation_latest.json`, `data/social_sentiment_latest.json`, `data/fx_exposure_latest.json`, `data/upcoming_events_latest.json`, `data/events_latest.json`.*
