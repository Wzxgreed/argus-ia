...
> **Date :** 2026-06-15 (snapshot 21h UTC)
> **Type :** `_update.md`
> **Source données :** `data/2026-06-15.json` (fetched_at 2026-06-15T21:00:02Z) + `data/recommandations_latest.json` + `data/social_sentiment_latest.json` + `data/fx_exposure_latest.json` + `data/upcoming_events_latest.json`
> **Statut pipeline :** 25/29 tickers OK — 🔴 [ERROR] QTBS confirmé snapshot 21h UTC

---

## 1. Résumé des changements depuis l'analyse précédente

| Item | Snapshot 17h UTC (15/06) | Snapshot 21h UTC (15/06) | Changement |
|------|--------------------------|--------------------------|------------|
...
| Timestamp erreur | 17:00:23Z | **21:00:20Z** | **Fetch ré-échoué** |

**Verdict macro :** QTBS reste en **data blackout total** au snapshot 21h UTC du 15/06. Aucune mutation vs les snapshots 10h et 17h UTC — **51e snapshot consécutif sans changement** depuis au moins le 20 mai 2026. Le rapport de validation (`validation_report.txt`, 2026-06-15T20:07:20Z) liste explicitement `[ERROR] QTBS: fetch failed — No price history` parmi 5 erreurs globales. Le seul delta technique est le timestamp du fetch échoué, passé de 17:00:23Z à 21:00:20Z, confirmant que le worker a bien été ré-exécuté sans succès.
...
- **Cours close :** [UNSOURCED] — `data/2026-06-15.json` (21h UTC) retourne `"error": true`, `"reason": "No price history"` (timestamp 2026-06-15T21:00:20Z)
- **RSI 14j :** —
- **ATR 14j :** —
- **MM 50j / 200j :** —
...
**Impact :** Sans cours et sans ATR, les niveaux de stop-loss et take-profit restent **non calculables**. Le timing technique est indéterminable. L'erreur est stable sur 51 snapshots consécutifs, excluant toute hypothèse de latence temporaire.
...
Aucune mention retail, aucune détection de pump/dump, aucune news Yahoo, aucun événement corporate. Le silence informationnel reste total. Le ticker n'apparaît dans aucun des rapports agents actifs (quant, geo, accounting, sector, events). L'entrée FX exposure (25% export, impact neutre) est inchangée depuis le snapshot 17h.
...
1. **Data blackout persistant :** Le snapshot 21h UTC du 15/06 confirme l'erreur `[ERROR] QTBS: fetch failed — No price history`. Ce bloquant empêche toute analyse technique, fondamentale et quantitative. La stabilité de l'erreur sur **51 snapshots consécutifs** confirme une panne structurelle du data feed, et non un effet de latence temporaire.

2. **Earnings J0 non analysable :** L'événement earnings du 2026-06-15 (source FMP, severity high, days_until=0) est un **placeholder glissant** resté figé au 15/06 entre 10h, 17h et 21h UTC. Sans cours pré-event, sans consensus EPS/Revenue et sans métriques, il est impossible de mesurer un quelconque impact post-announcement.
...
4. **Aucun nouveau détail contextuel :** Le rapport FX exposure (25% export, impact neutre), le sentiment social (0 mentions) et les événements corporate (aucun) sont tous inchangés vs le snapshot 17h. Aucun catalyseur externe n'est détecté.
...
*Rapport généré le 2026-06-15. Données : 2026-06-15.json (21h UTC), recommandations_latest.json (QTBS score 55.2), social_sentiment_latest.json (0 mentions), fx_exposure_latest.json (25% export, impact 0.0), upcoming_events_latest.json (J0 placeholder), validation_report.txt (5 erreurs globales dont QTBS).*  
