...
> **Date :** 2026-06-09 (close officiel 21h UTC)
> **Type :** `_update.md`
> **Source données :** `data/latest.json` (fetched_at 2026-06-09T21:00:02Z) + `data/recommandations_latest.json` + `data/quant_report_latest.json` + `data/geo_risk_latest.json` + `data/sector_rotation_latest.json` + `data/social_sentiment_latest.json` + `data/fx_exposure_latest.json` + `data/upcoming_events_latest.json` + `data/events_latest.json` + `data/validation_report.txt`
> **Statut pipeline :** 25/29 tickers OK — 🔴 [ERROR] QTBS confirmé close 21h UTC

---

## 1. Résumé des changements depuis l'analyse précédente

| Item | Snapshot 13h UTC (09/06) | Close 21h UTC (09/06) | Changement |
|------|--------------------------|-----------------------|------------|
...
**Verdict macro :** QTBS reste en **data blackout total** au close officiel 21h UTC du 09/06. Aucune mutation vs le snapshot 13h UTC du même jour — **43e snapshot consécutif sans changement** depuis au moins le 20 mai 2026. Le placeholder FMP reste figé au 09/06, confirmant son caractère **automatique et non une date d'earnings réelle**. Le rapport de validation du jour (`data/validation_report.txt`, 2026-06-09T20:07:20Z) liste explicitement `[ERROR] QTBS: fetch failed — No price history`.
...
- **Cours close :** [UNSOURCED] — `data/latest.json` retourne `"error": true`, `"reason": "No price history"` (timestamp 2026-06-09T21:00:18Z)
- **RSI 14j :** —
- **ATR 14j :** —
- **MM 50j / 200j :** —
...
**Impact :** Sans cours et sans ATR, les niveaux de stop-loss et take-profit restent **non calculables**. Le timing technique est indéterminable. L'erreur est stable sur 43 snapshots consécutifs, excluant toute hypothèse de latence temporaire.
...
- **Social sentiment (Reddit) :** No data — `mention_count: 0`, `sentiment_score: 0.0`, `pump_detected: false` (`data/social_sentiment_latest.json`, 2026-06-09)
...
**Rotation sectorielle :** Signal `NEUTRAL` détecté (`data/sector_rotation_latest.json`, 2026-06-09). QTBS n'est pas classé dans les secteurs SPDR scannés. Aucun impact direct mesurable.
...
1. **Data blackout persistant :** Le close officiel 21h UTC du 09/06 confirme l'erreur `[ERROR] QTBS: fetch failed — No price history`. Ce bloquant empêche toute analyse technique, fondamentale et quantitative. La stabilité de l'erreur sur **43 snapshots consécutifs** confirme une panne structurelle du data feed, et non un effet de latence temporaire.

2. **Earnings J0 non analysable :** L'événement earnings du 2026-06-09 (source FMP, severity high, days_until=0) est un **placeholder glissant** resté figé au 09/06 entre les snapshots 13h et 21h. Sans cours pré-event, sans consensus EPS/Revenue et sans métriques, il est impossible de mesurer un quelconque impact post-announcement.
...
*Rapport généré le 2026-06-09. Données : latest.json (21h UTC), recommandations_latest.json, quant_report_latest.json, geo_risk_latest.json, sector_rotation_latest.json, social_sentiment_latest.json, fx_exposure_latest.json, upcoming_events_latest.json, events_latest.json, validation_report.txt.*
