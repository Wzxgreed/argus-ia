...
> **Source :** data/latest.json (fetched_at 2026-06-08T13:00:01 UTC), data/recommandations_latest.json, data/social_sentiment_latest.json, data/fx_exposure_latest.json, data/upcoming_events_latest.json, data/events_latest.json, data/validation_report.txt
...
| Élément | 03/06 (10:00 UTC) | 08/06 (13:00 UTC) | Changement |
|---------|-------------------|-------------------|------------|
...
| ASTS (proxy) | **$118.17** | **$93.60** | **🔴 CORRECTION −12.76%** (stable vs 10h) |
| Volume ASTS | 21.29M (0.78×) | **23.90M (0.87×)** | **Stable sous moyenne** — +12.3% vs 03/06, identique 10h/13h |
| RSI ASTS | 72.58 | **54.36** | **🟢 SORTIE DU SURACHAT** (−18.2 pts, stable vs 10h) |
| ATR ASTS | 12.22 | **13.19** | **+7.9%** (stable vs 10h) |
| MM 50j ASTS | 87.67 | **88.42** | **+0.9%** — cours reste au-dessus (stable vs 10h) |
| Distance MM50j ASTS | +34.8% | **+5.9%** | Normalisation mécanique (stable vs 10h) |
| Low intraday ASTS | 108.80 | **90.905** | **Nouveau low** — cassure support $108.80 (stable vs 10h) |
| High intraday ASTS | 118.74 | **104.49** | **−12.0%** — résistance abaissée (stable vs 10h) |
...
| Premium vs consensus ASTS | **+25.0%** | **−1.0%** | **🟢 NORMALISATION MAJEURE** — cours sous consensus (stable vs 10h) |
| Score ASTS (agent) | 29.8/100 (ÉVITER) | **48.5/100 (ATTENDRE)** | **🔼 UPGRADE +18.7 pts** (stable vs 10h) |
| Timing ASTS (agent) | Défavorable | **Favorable** | **🟢 BASCULE** (stable vs 10h) |
| Score AST (agent) | 55.2/100 (ATTENDRE) | **55.2/100 (ATTENDRE)** | Placeholder stable |
| Options max pain ASTS | $40.0 (anomalie JSON 10h) | **$120.0** | **✅ ANOMALIE JSON RÉSOLUE** — valeur opérationnelle rétablie |
| Options put/call ASTS | null (données manquantes 10h) | **0.7** | **✅ RÉTABLI** — signal haussier (< 1.0) |
| Options call OI ASTS | null (données manquantes 10h) | **59.0%** | **✅ RÉTABLI** — majorité calls (> 50%) |
| Échéance options ASTS | 2026-06-05 | **2026-06-12** | **+4 jours** — stable vs 10h |
...
**Constat :** Le snapshot 13h UTC confirme la **stabilité totale** des données de marché ASTS vs le snapshot 10h (mêmes cours, RSI, volumes, consensus). La seule évolution significative est la **résolution de l'anomalie options JSON** : max pain revient à **$120.0** (cohérent), put/call ratio à **0.7** (haussier) et call OI à **59.0%** (majoritaire). Ces données rétablies renforcent le positionnement options haussier malgré la correction de −12.76%. AST reste sans aucune donnée de cours (>31 snapshots consécutifs).
...
| Indicateur | Valeur 13:00 UTC (08/06) | Valeur précédente (03/06) | Δ |
|-----------|-------------------------|---------------------------|---|
...
| Métrique | Valeur 13:00 UTC (08/06) | Valeur précédente | Δ |
|---------|-------------------------|-------------------|---|
...
| Métrique | Valeur 13:00 UTC (08/06) | Valeur précédente (03/06) | Δ |
|---------|-------------------------|---------------------------|---|
...
- **Options ASTS — ANOMALIE JSON RÉSOLUE ✅ :**
  - Max pain **$120.0** dans `data/latest.json` (snapshot 13h UTC) — **résolution confirmée** vs anomalie $45.0 du snapshot 10h. Cette valeur est cohérente avec l'historique opérationnel antérieur (max pain $120.0 observé le 26–27/05).
  - Put/call ratio **0.7** (rétabli) — **🟢 SIGNAL HAUSSIER** : ratio < 1.0 indique un excès de calls vs puts
  - Call OI **59.0%** (rétabli) — **🟢 MAJORITÉ CALLS** : open interest calls > 50% confirme le biais haussier du positionnement
  - Échéance prochaine : **2026-06-12** (dans 4 jours)
  - **Interprétation :** le rétablissement des données options révèle un positionnement net haussier (put/call 0.7, call OI 59.0%) malgré la correction de −12.76%. Le max pain à **$120.0** (+28.2% au-dessus du cours) constitue un aimant gamma distant mais réactivé. L'échéance 2026-06-12 dans 4 jours maintient un risque gamma significatif : si le cours se rapproche de $120, la pression de réachet gamma par les dealers pourrait amplifier les mouvements.
...
- **Upgrades/downgrades ASTS :** 12 analystes, price target moyen $94.54 — cours actuel $93.60 = **−1.0% sous le consensus** (normalisation vs +25.0% précédemment). Publishers FMP simplifiés : TheFly + StreetInsider (vs 5 publishers à 10h).
...
| Axe | Score 13:00 UTC (08/06) | Pondération | Note |
|-----|------------------------|-------------|------|
...
| Momentum | 6.0/10 | 25% | Correction −12.76% mais RSI normalisé à 54.36, cours au-dessus MM50j, options haussières (P/C 0.7, call OI 59%) |
...
> ASTS n'est PAS dans le périmètre d'analyse officiel d'AST. Ces scores sont fournis uniquement pour quantifier l'évolution du proxy. L'**upgrade de ÉVITER (29.8/100) à ATTENDRE (48.5/100)** reflète : (1) la **sortie du surachat technique** (RSI 54.36 vs 72.58), (2) la **normalisation du premium consensus** (−1.0% vs +25.0%), et (3) le **maintien au-dessus de la MM50j** ($88.42). La résolution de l'anomalie options JSON (put/call 0.7, call OI 59.0%) n'a pas modifié le score car le positionnement haussier options était déjà implicitement pricé dans la volatilité. Cependant, le score Opportunité reste sous le seuil d'achat (4.8/10 < 6.0/10) et la valorisation demeure spéculative (EV/Revenue 335.6×, forward P/E −456.14). L'action reste classée **ATTENDRE**.
...
> ASTS n'est PAS dans le périmètre d'analyse officiel d'AST. Ces niveaux sont fournis uniquement pour quantifier la volatilité du proxy. Les niveaux sont **identiques au snapshot 10h** (stabilité totale). Le support immédiat est désormais la **MM50j à $88.42** ; une cassure ouvrirait la voie vers la zone **$80–85** puis le low du jour à **$90.905** (déjà testé). La résistance immédiate est la zone **$103.33–104.49** (gap baissier). Un franchissement durable au-dessus de **$104.50** réactiverait le biais haussier. L'échéance options 2026-06-12 (dans 4 jours) avec max pain $120.0 (+28.2% du cours) maintient un risque gamma à surveiller : le positionnement haussier (P/C 0.7, call OI 59%) pourrait générer une pression de réachet si le cours remonte vers $100–104.
...
**Thèse ASTS (proxy) :** 🟡 **CONFIRMÉE — CORRECTION MAJEURE ET NORMALISATION, RESTE ATTENDRE, OPTIONS RÉTABLIES**
...
2. **Correction majeure sur ASTS confirmée stable :** le cours a chuté de **−12.76%** en séance pour clôturer à **$93.60**, avec un low à **$90.905** et un gap baissier à l'ouverture ($103.33 vs $107.29). Le snapshot 13h UTC confirme la **stabilité totale** vs 10h (mêmes cours, RSI, volumes, consensus).
3. **Normalisation du premium consensus confirmée :** le cours est revenu **légèrement sous le consensus analystes** ($93.60 vs $94.54, premium **−1.0%**) pour la première fois depuis plusieurs semaines. Cette normalisation élimine le principal argument de surévaluation.
4. **Sortie du surachat technique confirmée :** le RSI est passé de **72.58 à 54.36** (zone neutre favorable). Cette sortie du surachat, combinée au retour sous le consensus, explique l'**upgrade agent ÉVITER → ATTENDRE (+18.7 pts)**.
5. **Anomalie options JSON RÉSOLUE ✅ :** le snapshot 13h UTC corrige l'anomalie du 10h (max pain $45.0 aberrant → **$120.0** cohérent). Le put/call ratio est rétabli à **0.7** (haussier) et le call OI à **59.0%** (majoritaire). Ce positionnement options haussier malgré la correction de −12.76% est un signal à surveiller : les acheteurs d'options anticipent un rebond.
6. **Support / Résistance revus :** le support immédiat est désormais la **MM50j à $88.42** (cours actuel +5.9%). Le low intra-day à **$90.905** a déjà été testé. La résistance immédiate est la zone **$103.33–104.49** (gap baissier). Un franchissement durable au-dessus de **$104.50** réactiverait le biais haussier ; une cassure sous **$88.42** (MM50j) ouvrirait la voie vers **$80–85**.
7. **Volume sous moyenne mais en hausse :** le volume à **23.90M (0.87×)** est resté sous la moyenne 20j (27.50M) mais en progression de +12.3% vs le 03/06. Cela suggère une vente ordonnée plutôt qu'un panic selling massif.
8. **Risque gamma réactivé :** l'échéance options **2026-06-12** (dans 4 jours) avec max pain **$120.0** (+28.2% du cours) et positionnement haussier (P/C 0.7, call OI 59%) crée un setup asymétrique. Si le cours remonte vers $100–104, la pression de réachet gamma par les dealers pourrait amplifier le mouvement. À l'inverse, une cassure sous $90 renforcerait la pression vendeuse.
9. **Agent maintient ATTENDRE avec timing Favorable :** l'agent a confirmé ASTS à **ATTENDRE (48.5/100)** avec un timing **Favorable**. Cependant, le score Opportunité (4.8/10) reste sous le seuil d'achat (6.0/10). La valorisation spéculative (EV/Revenue 335.6×, forward P/E −456.14) et l'absence de catalyseur court terme justifient le maintien en ATTENDRE.
10. **Earnings placeholder glissant non résolu :** FMP signale un earnings AST le **2026-06-08** (`days_until: 0`), mais sans historique de prix, le résultat ne peut être corrélé à un mouvement de marché. Le glissement J=0 persiste depuis le **25/05** (14+ jours de décalage non résolu).
