...
| MM 50j | $143.54 | **$142.64** | **−$0.90 (−0.6%)** |
| Volume du jour | 29.70M vs 43.53M avg (−31.8%) | **27.48M vs 40.64M avg (−32.4%)** | **Compression identique** |
| Short Interest | 0.03% | **2.77%** | **+2.74 pts** |
| Consensus FMP PT | $187.61 (33 analystes) | **$186.15 (34 analystes, +1 analyste)** | **−$1.46 (−0.8%)** |
| Upside vs PT | +38.7% | **+35.9%** | **−2.8 pts** |
| Put/Call Ratio | 0.61 | **0.48** | **−0.13 (−21.3%)** |
| Max Pain | $140.00 | **$140.00** | **0** |
| Call OI % | 62.2% | **67.4%** | **+5.2 pts** |
| Score Opportunité agent | 5.4/10 | **5.1/10** | **−0.3 pt** |
...
|-----------|--------|--------|
| Cours | $136.88 | −0.39% session ; +1.20% vs close 20/05 |
| RSI 14j | 35.66 | 🔴 **Survente** — franchissement zone 40 depuis le 20/05 |
| ATR 14j | $5.35 | Volatilité en légère contraction (−4.8% vs 20/05) |
...
| Support clé | $134.30 | Low du jour — zone de défense immédiate |
...
**Options — Données complètes et cohérentes :**

| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| Put/Call Ratio | **0.48** | 🟢 Haussier modéré — renforcement du biais call vs 0.61 précédent |
| Max Pain | **$140.00** | Réalignement avec le cours — pinning mécanique probable autour de $140.00 à expiration |
| Call OI % | **67.4%** | Appétence haussière modérée, en hausse de 5.2 pp |
| Expiration proche | 2026-05-29 | Dans 4 jours — gamma risk concentré autour de $140.00 |

**Interprétation technique :**
- Le cours a légèrement progressé (+1.2%) entre le 20/05 et le 25/05, mais la dynamique technique s'est dégradée avec le **franchissement du RSI sous 40 (35.66)**. Cela signale un retour en zone de survente et un affaiblissement du momentum.
...
- **Max Pain $140.00 vs cours $136.88** : le cours est légèrement sous le max pain à 4 jours de l'expiration. Le marché options anticipe une gravitation vers $140.00, ce qui constitue un objectif technique plausible si un rebond se matérialise.
...
- **Niveau critique : $134.30** (low du 25/05). Cassure sous ce niveau = test du support $130 puis $126.18 (2×ATR).
...
- **Price Target moyen FMP : $186.15** (34 analystes, **+1 analyste vs précédent**, 5 mises à jour le mois dernier, 6 le trimestre dernier)
- **Upside implicite : +35.9%** vs cours $136.88 (vs +38.7% le 20/05)
- **Couverture :** 34 analystes — coverage significatif et actif
...
### Options
- **Put/Call 0.48** (vs 0.61 le 20/05) : biais haussier modérément renforcé. Le marché options anticipe un rebond ou maintient des positions haussières malgré la dégradation technique.
- **Max Pain $140.00** (inchangé) : cohérent avec le spot $136.88. Zone de gravitation options à +2.3%.
- **Call OI 67.4%** : appétence haussier en hausse de 5.2 pp. La structure options constitue un facteur mitigant face au RSI survente.
...
| Momentum | 5.0 | **3.5** | **−1.5** | RSI 35.66 = survente. Volume compressé −32.4%. Sous MM50. Dynamique baissière renforcée. |
| **Score Opportunité** | **5.4** | **5.1** | **−0.3** | Pondération 35/40/25 (régime inconnu = default) |
...
|---|------------|------------|---------------|
| Entrée suggérée | $135.26 | **$136.88** | Close actuel — **Ne pas entrer à ce niveau** |
| Stop-Loss | $124.02 | **$126.18** | Cours − 2×ATR = $136.88 − $10.70. Aligné sur support technique |
| Take-Profit | $152.12 | **$152.93** | Cours + 3×ATR = $136.88 + $16.05. Objectif technique sous consensus PT |
| Ratio R/R | 1.5 | **1.5** | — |

**Note institutionnelle :** Les niveaux ont été révisés mécaniquement à la hausse suite à la légère progression du cours (+1.2%) et à la contraction de l'ATR. Le SL $126.18 correspond à la zone $126–$130 (support technique post-rally). Une cassure sous $126.18 en clôture = invalidation du trend neutre et risque de retour vers $118.93 (52W low). Le TP $152.93 reste conservateur. Si le cours franchit $142.64 (MM50) sur volume > 40M, le TP peut être révisé vers $160–$165. **Expiration options 29/05 dans 4 jours** : le Max Pain $140.00 vs cours $136.88 indique un potentiel de rebond mécanique de +2.3% si le gamma call se décharge.
...
4. **Options biais haussier renforcé** — Put/Call 0.61 → 0.48, Call OI 62.2% → 67.4%. Facteur mitigant.
5. **Score Momentum 5.0 → 3.5** — Dégradation due au RSI survente et au volume compressé.
6. **Score Global ajusté 46.3 → 42.5** — Conséquence mécanique de la baisse du momentum.
...
3. **Gamma risk à expiration 29/05** — Dans 4 jours. Max Pain $140.00 vs cours $136.88 = potentiel de rebond mécanique de +2.3% si le momentum call se maintient.
...
- `data/upcoming_events_2026-05-25.json` — Earnings 2026-08-03, 70 jours
...
- `data/geo_risk_2026-05-25.json` — Score Politique 0/10, non exposé
