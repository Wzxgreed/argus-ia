# SPCX — Mise à jour post-pipeline 2026-06-10 (snapshot 10h UTC)

**Date :** 2026-06-10
**Type :** Mise à jour post-pipeline — snapshot 10h UTC
**Analyse précédente :** close officiel 21h UTC 2026-06-09

---

## Résumé des changements depuis l'analyse précédente

| Donnée | Précédent (21h UTC 09/06) | Actuel (10h UTC 10/06) | Changement |
|--------|--------------------------|------------------------|------------|
...
| Signal sectoriel | NEUTRAL (données corrompues — NaN) | NEUTRAL (données corrompues — NaN) | = |

**Verdict :** Dix-huitième snapshot consécutif sans données fiables. **Stabilité totale** vs le close officiel 21h UTC du 09/06 : cours inchangé, volume inchangé, scores inchangés, conflit de symbole FMP persistant. Aucune mutation détectée entre les deux snapshots.
...
**Verdict timing :** Défavorable → **Non-actionnable**. L'absence de données techniques fiables (RSI, ATR, MM50) sur dix-huit snapshots consécutifs, combinée à un cours manifestement erroné ($135.00 avec open $0.00), rend toute analyse technique impossible.
...
|--------|------|-------------|
| News | Aucune structurante | `data/news_2026-06-10.json` : 0 item pour tous les tickers (source yahoo_rest) |
| Social sentiment | No data | `data/social_sentiment_2026-06-10.json` : 0 mentions Reddit, pump_detected = false |
...
| FX Exposure | 🟢 | `data/fx_exposure_2026-06-10.json` : fx_impact_score 0.0, flag 🟢, neutral |
...
**Anomalie data quality persistante :** `data/upcoming_events_2026-06-10.json` mentionne un faux événement `earnings` pour SPCX le 2026-06-10 (source FMP, days_until = 0) — artefact connu pour un ETF, à ignorer. Ce faux signal est récurrent depuis plusieurs snapshots et n'impacte pas le scoring.
...
**Alerte validation :** `data/validation_report.txt` (2026-06-10) signale un `[WARNING] SPCX: volume is 0 — market closed or delisted?`

---

## Scoring global (agents pipeline 2026-06-10, snapshot 10h UTC)

| Axe | Score | Changement vs 21h 09/06 | Commentaire |
|-----|-------|------------------------|-------------|
...
- **Sector rotation :** +0 (données corrompues — NaN sur tous les secteurs, momentum_score 10.0 mécanique)
...
**Verdict :** 🔴 Thèse **CONFIRMÉE** en état **SURVEILLER** — dix-huit snapshots consécutifs sans données fiables, conflit de symbole FMP persistant, **stabilité totale du scoring** (Score Global 47.2/100 inchangé). Le setup reste non-actionnable.
...
| Signal sectoriel | 🔴 **Données corrompues** : sector rotation retourne NaN sur tous les returns, momentum_score mécanique 10.0 pour tous les secteurs — ininterprétable |
...
| Validation report | 🟡 `[WARNING] SPCX: volume is 0 — market closed or delisted?` |
...
- **Nuances :** Le snapshot 10h UTC du 10/06 confirme la **stabilité totale** par rapport au close officiel 21h UTC du 09/06 : cours identique ($135.00), volume identique (0), secteur erroné identique (`Industrials` / `Aerospace & Defense`), scores identiques (Global 47.2, Opportunité 4.7, Catalyseur 6.5, Valorisation 3.0, Momentum 5.0). Le module sector rotation est désormais **totalement corrompu** (returns NaN, momentum_score 10.0 uniforme sur 11 secteurs) — le signal NEUTRAL précédent n'est plus vérifiable. Aucune news, événement corporate, flux options ni social n'est détecté sur ce snapshot. Le faux événement FMP `earnings` du 10/06 est un artefact récurrent et ignoré. L'alerte `EXTREME_BEARISH` du module social est un artefact mécanique (0 mention) et ignorée. Le validation report signale un WARNING sur le volume 0.
...
**Horizon :** —
**Conviction :** Très faible — setup technique suspendu par absence totale de données fiables sur dix-huit snapshots consécutifs. Le flux Yahoo reste indisponible et FMP semble renvoyer les données d'une autre ticker (conflit de symbole). Attendre un snapshot avec prix confirmé, sector correct et volume > 0 avant toute réévaluation.
...
| XLF momentum_score > 6.0 (sur données fiables) | 5–10j | Contexte sectoriel favorable | — |
...
- Raison principale : Snapshot 10h UTC 10/06 : stabilité totale vs close officiel 21h UTC 09/06, dix-huit snapshots consécutifs sans données fiables, conflit de symbole FMP persistant (cours $135.00, OHLC $0.00/$135.00, volume 0, sector Industrials/Aerospace au lieu de Financial Services/Asset Management). Scoring inchangé : Score Global 47.2/100, Score Opportunité 4.7/10 (C:6.5 V:3.0 M:5.0). Signal sectoriel NEUTRAL mais données corrompues (NaN sur tous les returns, momentum_score 10.0 mécanique). Aucun catalyseur ni news. Faux earnings FMP du 10/06 ignoré (artefact récurrent). Alerte social sentiment EXTREME_BEARISH ignorée (artefact mécanique, 0 mention). Validation report WARNING volume 0.
