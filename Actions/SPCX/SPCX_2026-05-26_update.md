# SPCX (SPAC ETF) — Mise à jour post-pipeline 2026-05-26 (snapshot 21:00 UTC)

**Date :** 2026-05-26
**Type :** Mise à jour post-pipeline — snapshot 21:00 UTC
**Analyse précédente :** [SPCX_2026-05-26_update.md](./SPCX_2026-05-26_update.md) (snapshot 17:00 UTC)

---

## Résumé des changements depuis l'analyse précédente

| Donnée | Précédent (17:00 UTC 26/05) | Actuel (21:00 UTC 26/05) | Changement |
|--------|----------------------------|--------------------------|------------|
...
**Verdict :** Stabilité totale confirmée vs snapshot 17:00 UTC 26/05. C'est le **6e snapshot consécutif sans mutation** (10:00 UTC 25/05 → 21:00 UTC 25/05 → 10:00 UTC 26/05 → 13:00 UTC 26/05 → 17:00 UTC 26/05 → 21:00 UTC 26/05). Sur l'ensemble de la séance US du 26/05 et sur les deux derniers jours, SPCX n'enregistre aucune mutation de prix, de volume ni de métrique technique entre les snapshots. Cette stabilité persistante confirme un ancrage technique exceptionnel à $22.40.

---

## Mise à jour technique

Aucune mutation vs snapshot 17:00 UTC — données Yahoo strictement identiques sur le snapshot 21:00 UTC.
...
| Volume vs moy. 20j | 4.48× | 🔴 Anomalie volume — accumulation ou repositionnement confirmé sur 6 snapshots |
...
**Verdict timing :** Favorable. Le setup technique (au-dessus MM50, RSI 62.4, volume anormal ×4.5) reste validé par l'Agent Recommandation. L'ATR de $0.27 confirme un range serré — le risque de gap est limité. La stabilité sur 6 snapshots consécutifs renforce la fiabilité du niveau $22.40 comme ancrage technique fiable.
...
**Sector rotation :** Le secteur Financials (XLF) n'apparaît pas dans le top3 ni le bottom3 du `sector_rotation_latest.json` daté 2026-05-26. XLF enregistre un return_20d de 0.08% et un momentum_score de 0.0 — pas de bonus/malus sectoriel pour SPCX. Seul XLE (Energy) affiche un bullish crossover, sans lien direct avec le secteur Financials.
...
## Scoring global (agents pipeline 2026-05-26, snapshot 21:00 UTC)

| Axe | Score | Changement vs 17:00 UTC 26/05 | Commentaire |
|-----|-------|------------------------------|-------------|
...
- **Timing technique :** +10 (cours au-dessus MM50 + volume anormal confirmé sur 6 snapshots)
...
La recommandation reste **ACHETER (Réduit)** — niveaux confirmés par Agent Recommandation, inchangés vs 17:00 UTC 26/05.
...
**Verdict :** 🟢 Thèse **CONFIRMÉE** — 6e snapshot consécutif sans mutation (10:00 UTC 25/05 → 21:00 UTC 25/05 → 10:00 UTC 26/05 → 13:00 UTC 26/05 → 17:00 UTC 26/05 → 21:00 UTC 26/05)

| Critère | Évaluation |
|---------|------------|
| Cours vs MM50 | ✅ Au-dessus ($22.40 > $21.99) |
| RSI | ✅ Haussier (62.4) — pas de surachat |
| Volume | 🟢 Anomalie haussière confirmée (4.48× moyenne) sur 6 snapshots |
...
| Stabilité snapshots | 🟢 6e snapshot consécutif identique — fiabilité renforcée sur 2 séances complètes |

- **Confirmation :** Le setup technique identifié le 25/05 (retour au-dessus MM50, volume record, RSI 62.4) est validé par la stabilité des données sur 6 snapshots consécutifs, incluant deux séances complètes de marché ouvert. L'absence de mutation ce 26/05 à 21:00 UTC confirme que $22.40 est le niveau de référence fiable.
...
**Conviction :** Modérée — setup technique validé par les agents et confirmé par la stabilité des snapshots sur 6 sessions consécutives (incluant deux séances complètes), mais manque de catalyseur fondamental et faible liquidité historique. Sizing réduit obligatoire.
...
|--------|----------------|-----------|----------------|
| Volume journalier | 4.48× moy. 20j | 🔴 Anomalie | Signal d'accumulation confirmé par Agent Recommandation — stable sur 6 snapshots |
...
- Analyse précédente : [SPCX_2026-05-26_update.md](./SPCX_2026-05-26_update.md) (snapshot 17:00 UTC 26/05)
...
- Raison principale : Snapshot 21:00 UTC 26/05 confirme stabilité totale vs 17:00 UTC 26/05 — 6e snapshot consécutif sans mutation, thèse CONFIRMÉE
- Thèse : 🟢 Confirmée
