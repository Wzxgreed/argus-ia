# SPCX (SPAC ETF) — Mise à jour post-pipeline 2026-05-26 (snapshot 17:00 UTC)

**Date :** 2026-05-26
**Type :** Mise à jour post-pipeline — snapshot 17:00 UTC
**Analyse précédente :** [SPCX_2026-05-26_update.md](./SPCX_2026-05-26_update.md) (snapshot 13:00 UTC)

---

## Résumé des changements depuis l'analyse précédente

| Donnée | Précédent (13:00 UTC 26/05) | Actuel (17:00 UTC 26/05) | Changement |
|--------|----------------------------|--------------------------|------------|
...
**Verdict :** Stabilité totale confirmée vs snapshot 13:00 UTC 26/05. C'est le **5e snapshot consécutif sans mutation** (10:00 UTC 25/05 → 21:00 UTC 25/05 → 10:00 UTC 26/05 → 13:00 UTC 26/05 → 17:00 UTC 26/05). Sur une journée de marché US entièrement ouverte post-Memorial Day, SPCX n'enregistre aucune mutation de prix, de volume ni de métrique technique. Cette stabilité persistante sur une séance complète confirme un range exceptionnellement serré et un ancrage technique fiable à $22.40.

---

## Mise à jour technique

Aucune mutation vs snapshot précédent — données Yahoo strictement identiques malgré le marché US ouvert en continu sur la séance entière du 26/05.
...
| Volume vs moy. 20j | 4.48× | 🔴 Anomalie volume — accumulation ou repositionnement confirmé sur 5 snapshots |
...
| Open / High / Low | $22.76 / $22.76 / $22.29 | Range intraday $0.47 (2.1%) — clôture médiane |
...
**Verdict timing :** Favorable. Le setup technique (au-dessus MM50, RSI 62.4, volume anormal ×4.5) reste validé par l'Agent Recommandation. L'ATR de $0.27 confirme un range serré — le risque de gap est limité. La stabilité sur 5 snapshots consécutifs, incluant une séance complète post-Memorial Day, renforce la fiabilité du niveau $22.40 comme ancrage technique.
...
**Sector rotation :** Le secteur Financials (XLF) n'apparaît pas dans le top3 ni le bottom3 du `sector_rotation_latest.json` daté 2026-05-26. XLF enregre un return_20d de 0.0% et un momentum_score de 0.0 — pas de bonus/malus sectoriel pour SPCX. Seul XLE (Energy) affiche un bullish crossover, sans lien direct avec le secteur Financials.
...
|--------|------|-------------|
| News | Aucune structurante | `data/events_latest.json` (2026-05-26) = 0 événement corporate pour SPCX |
...
**Anomalie data quality persistante :** `data/upcoming_events_latest.json` (2026-05-26) mentionne un faux événement `earnings` pour SPCX (source FMP, days_until = 0) — artefact connu, à ignorer pour un ETF.

---

## Scoring global (agents pipeline 2026-05-26, snapshot 17:00 UTC)

| Axe | Score | Changement vs 13:00 UTC 26/05 | Commentaire |
|-----|-------|------------------------------|-------------|
...
- **Timing technique :** +10 (cours au-dessus MM50 + volume anormal confirmé sur 5 snapshots)
...
La recommandation reste **ACHETER (Réduit)** — niveaux confirmés par Agent Recommandation, inchangés vs 13:00 UTC 26/05.
...
**Verdict :** 🟢 Thèse **CONFIRMÉE** — 5e snapshot consécutif sans mutation (10:00 UTC 25/05 → 21:00 UTC 25/05 → 10:00 UTC 26/05 → 13:00 UTC 26/05 → 17:00 UTC 26/05)

| Critère | Évaluation |
|---------|------------|
| Cours vs MM50 | ✅ Au-dessus ($22.40 > $21.99) |
| RSI | ✅ Haussier (62.4) — pas de surachat |
| Volume | 🟢 Anomalie haussière confirmée (4.48× moyenne) sur 5 snapshots |
...
| Stabilité snapshots | 🟢 5e snapshot consécutif identique — fiabilité renforcée sur séance complète post-Memorial Day |

- **Confirmation :** Le setup technique identifié le 25/05 (retour au-dessus MM50, volume record, RSI 62.4) est validé par la stabilité des données sur 5 snapshots consécutifs, incluant une séance complète de marché ouvert post-Memorial Day. L'absence de mutation ce 26/05 à 17:00 UTC confirme que $22.40 est le niveau de référence fiable.
- **Nuances :** Le mouvement reste 100% technique. L'absence de news fondamentale ou de catalyseur sectoriel (reprise SPAC/IPO, baisse des taux) limite la conviction et justifie le sizing Réduit. Le secteur Financials (XLF) n'est pas dans la rotation haussière du jour (momentum_score = 0.0).
...
**Conviction :** Modérée — setup technique validé par les agents et confirmé par la stabilité des snapshots sur 5 sessions consécutives (incluant une séance complète), mais manque de catalyseur fondamental et faible liquidité historique. Sizing réduit obligatoire.
...
|--------|----------------|-----------|----------------|
| Volume journalier | 4.48× moy. 20j | 🔴 Anomalie | Signal d'accumulation confirmé par Agent Recommandation — stable sur 5 snapshots consécutifs |
...
- Analyse précédente : [SPCX_2026-05-26_update.md](./SPCX_2026-05-26_update.md) (snapshot 13:00 UTC 26/05)
...
- Raison principale : Snapshot 17:00 UTC 26/05 confirme stabilité totale vs 13:00 UTC 26/05 — 5e snapshot consécutif sans mutation, thèse CONFIRMÉE
- Thèse : 🟢 Confirmée
