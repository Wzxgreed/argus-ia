# SPCX (SPAC ETF) — Mise à jour post-pipeline 2026-06-01 (snapshot 21:00 UTC)

**Date :** 2026-06-01
**Type :** Mise à jour post-pipeline — snapshot 21:00 UTC
**Analyse précédente :** snapshot 17:00 UTC 2026-06-01

---

## Résumé des changements depuis l'analyse précédente

| Donnée | Précédent (17:00 UTC 01/06) | Actuel (21:00 UTC 01/06) | Changement |
|--------|----------------------------|--------------------------|------------|
...
**Verdict :** Aucune mutation technique, de cours, ni de sentiment entre les snapshots 17h et 21h UTC. La persistance des données Yahoo absentes (RSI, ATR, MM50) et du volume quasi nul (196 unités) sur l'ensemble de la séance confirme le caractère **non-actionnable** du setup. Recommandation **ATTENDRE** maintenue sans changement.

---

## Mise à jour technique

**[DONNÉES PARTIELLES]** — Le bloc `technical` de `data/latest.json` pour SPCX reste vide (source : `fmp_fallback`). Aucune métrique technique Yahoo n'est disponible sur ce snapshot 21h.
...
| Volume journalier | 196 | 🔴 Quasi-illiquide — inchangé vs 17h |
...
**Verdict timing :** Neutre → Défavorable. L'absence totale de données Yahoo sur trois snapshots consécutifs (10h, 17h et 21h) et le volume figé à 196 unités rendent tout setup technique non vérifiable. Le range intraday de $0.03 confirme un marché totalement figé, sans participation institutionnelle ni retail.
...
**Sector rotation :** `data/sector_rotation_2026-06-01.json` enregistre un signal stable `ROTATION_TO_CYCLICAL`. XLF (Financials) figure dans le `top3` avec un `return_20d` de −0.94% et un `momentum_score` de 0.0 — pas de bonus/malus sectoriel pour SPCX. XLK (Technology) domine avec `momentum_score` 10.0 et `return_20d` +20.94%. Le secteur Financials reste sous-performant vs SPY sur 20j (`rs_20d` −6.20%).
...
## Scoring global (agents pipeline 2026-06-01, snapshot 21:00 UTC)
...
**Verdict :** 🟡 Thèse **CONFIRMÉE** en état ATTENDRE — aucune amélioration ni dégradation entre 17h et 21h, et la persistance des anomalies data sur trois snapshots renforce le statu quo non-actionnable.
...
|--------|----------------|-----------|----------------|
| Volume journalier | 196 | 🔴 Extrême anomalie | Inchangé sur 10h–21h — quasi-illiquide persistant, marché figé |
...
- Analyse précédente : snapshot 17:00 UTC 01/06
...
- Raison principale : Snapshot 21:00 UTC 01/06 : aucune mutation vs 17h — données Yahoo absentes (RSI/ATR/MM50), volume figé à 196 unités, source fmp_fallback persistante sur 3 snapshots, Score Global 54.0 maintenu, signal sectoriel ROTATION_TO_CYCLICAL sans impact concret
