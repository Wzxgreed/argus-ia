# RKLB — Mise à Jour Snapshot 13h UTC 2026-06-16

> Source : `data/latest.json` (snapshot 13h UTC) | `data/recommandations_latest.json` | `data/upcoming_events_latest.json` | `data/sector_rotation_latest.json` | `data/social_sentiment_latest.json` | `data/fx_exposure_latest.json`

---

## 1. Résumé des Changements depuis le Snapshot 10h UTC 16/06

| Métrique | Snapshot 10h UTC 16/06 | Snapshot 13h UTC 16/06 | Variation |
|---|---|---|---|
...
| **Volume séance** | 28,81 M (1,03×) | **28,81 M** (1,03×) | Inchangé — participation normale |
...
| **Max Pain (Yahoo)** | $35,00 (aberrant) | **$65,00** | **[ANOMALIE PARTIELLEMENT RÉSOLUE]** — valeurs opérationnelles du 21h UTC 15/06 conservées |
| **Put/Call ratio (Yahoo)** | null (corrompu) | **0,67** | **Rétabli** — valeurs opérationnelles du 21h UTC 15/06 : 0,74 |
| **Call OI % (Yahoo)** | null (corrompu) | **59,8 %** | **Rétabli** — valeurs opérationnelles du 21h UTC 15/06 : 57,6 % |
| **Earnings Q2 2026** | 51 jours | **51 jours** | Inchangé |

**Verdict** : **Stabilité mécanique totale** des données brutes et du scoring vs le snapshot 10h UTC du 16/06. Le snapshot 13h UTC du 16/06 reste un snapshot pré-ouverture US (marché fermé à 13h UTC / 9h EDT). Aucune nouvelle séance n'a eu lieu. L'amélioration notable concerne les **données options JSON partiellement rétablies** : Put/Call (0,67) et Call OI % (59,8 %) sont de nouveau disponibles, contre `null` dans le snapshot 10h UTC. Le Max Pain reste aberrant à $65,00 (vs $35,00 à 10h UTC) — les **valeurs opérationnelles du 21h UTC 15/06 (Max Pain $120,00, Put/Call 0,74, Call OI 57,6 %) restent la référence**.
...
**Verdict timing : Neutre à légèrement favorable** — La structure technique est inchangée vs le snapshot 10h UTC du 16/06. La MM50 ($101,57) reste un support solide avec +7,6 % de marge. Le RSI à 33,14 laisse de la place à un rebond si le sentiment global s'améliore. Cependant, sans catalyseur fondamental, la dynamique reste technique. Le marché US n'étant pas encore ouvert (snapshot 13h UTC), aucun nouveau signal technique n'est à signaler.

---

## 3. Mise à Jour Fondamentale

Aucune news fondamentale majeure détectée entre le 10h UTC et le 13h UTC 16/06. `data/news_latest.json` vide pour RKLB. `data/events_latest.json` vide (0 événement corporate).

| Métrique | Valeur | Variation vs 10h UTC 16/06 |
|---|---|---|
...
| Signal | Valeur | Évolution vs 10h UTC 16/06 |
|---|---|---|
| **Consensus analystes (FMP)** | $90,83 (18 analysts) | Inchangé |
| **Max Pain (Yahoo)** | $65,00 | **Amélioration mécanique** (vs $35,00 à 10h UTC) — reste aberrant |
| **Put/Call ratio (Yahoo)** | 0,67 | **Rétabli** (vs `null` à 10h UTC) — proche valeur opérationnelle 0,74 |
| **Call OI % (Yahoo)** | 59,8 % | **Rétabli** (vs `null` à 10h UTC) — proche valeur opérationnelle 57,6 % |
...
- **[ANOMALIE OPTIONS JSON AMÉLIORATION PARTIELLE]** — Le snapshot 13h UTC du 16/06 montre une résolution partielle de l'anomalie options : Put/Call 0,67 et Call OI 59,8 % sont rétablis (étaient `null` à 10h UTC). Le Max Pain est passé de $35,00 à $65,00 — toujours incohérent mais moins aberrant. **Les valeurs opérationnelles du 21h UTC 15/06 (Max Pain $120,00, Put/Call 0,74, Call OI 57,6 %) restent la référence jusqu'à confirmation d'une nouvelle source fiable.**
- **Aucune news**, aucun insider trade, aucun événement corporate détecté.
- **Social sentiment mort** — 0 mentions, pas de pump/dump.

**Verdict Sentiment : Neutre** — L'absence de news continue de dominer. L'amélioration mécanique des données options ne modifie pas la lecture du sentiment. Le consensus inchangé à $90,83 suggère que le sell-side maintient sa vue malgré la volatilité.
...
**Comparaison avec le snapshot 10h UTC 16/06** : Le scoring est **inchangé** à 47,0/100. Tous les piliers (Catalyseur, Valorisation, Momentum) sont stables. L'amélioration notable est la **résolution partielle de l'anomalie options JSON** dans `data/latest.json` (snapshot 13h UTC) : Put/Call et Call OI rétablis. Cette amélioration mécanique ne modifie pas les scores agents mais renforce la fiabilité des données brutes pour les prochains snapshots.
...
|---|---|---|
| **Prix de référence** | $109,25 (close 13h UTC 16/06) | — |
...
Le snapshot 13h UTC du 16/06 confirme la thèse de **SURVEILLER** établie au snapshot 10h UTC du 16/06, avec **stabilité mécanique totale** des données brutes et du scoring :

1. ✅ **Stabilité totale des données brutes** — Cours $109,25, RSI 33,14, ATR $12,04, MM50 $101,57, scores 47,0/100 inchangés vs 10h UTC 16/06.
2. 🟢 **Amélioration partielle des données options JSON** — Put/Call 0,67 et Call OI 59,8 % rétablis (vs `null` à 10h UTC). Max Pain remonté de $35,00 à $65,00 (toujours aberrant). Les données opérationnelles du 21h UTC 15/06 (Max Pain $120,00, Put/Call 0,74, Call OI 57,6 %) restent la référence.
3. 🟢 **MM50 intacte** — Spot $109,25 = +7,6 % au-dessus de la MM50 ($101,57). La tendance haussière structurelle n'est pas menacée.
4. 🟡 **RSI bas mais stable** — 33,14. La zone <30 = survente n'a pas été atteinte. Cela limite l'asymétrie haussière à court terme.
...
7. 🟢 **Aucun événement corporate** — `data/events_latest.json` vide pour RKLB. Aucun catalyseur externe.
