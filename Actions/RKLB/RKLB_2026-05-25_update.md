# RKLB — Mise à Jour Snapshot 21:00 UTC (2026-05-25)

> Source : `data/latest.json` (fetched 2026-05-25T21:00:11 UTC) | `data/recommandations_2026-05-25.json` | Marché US fermé Memorial Day

---

## 1. Résumé des Changements depuis l'Analyse Précédente

| Métrique | Snapshot 17:00 UTC | Snapshot 21:00 UTC | Variation |
|---|---|---|---|
...
| **Max Pain** | $73,00 [ANOMALIE] | **$73,00** | Inchangé — anomalie data persistante |
| **Put/Call ratio** | 1,00 | **1,00** | Inchangé |
| **Call OI %** | 50,0 % | **50,0 %** | Inchangé |

**Verdict** : Aucune variation entre les snapshots 17:00 UTC et 21:00 UTC — le marché US était fermé pour le Memorial Day. Les données de clôture du 25 mai restent les références opérationnelles. La thèse **ÉVITER** est confirmée sans modification.
...
**Niveaux clés révisés** (inchangés vs 17:00 UTC) :
...
- **Max Pain** (éch. 2026-05-29) : **$73,00** [ANOMALIE DATA] — incohérent vs spot
...
Aucune nouvelle donnée fondamentale. Marché fermé Memorial Day.

| Métrique | Valeur | Source | Évolution vs 17:00 UTC |
|---|---|---|---|
...
| **Put/Call ratio** | **1,00** | Inchangé — équilibre parfait |
| **Call OI %** | **50,0 %** | Inchangé — biais call neutralisé |
| **Max Pain (snapshot)** | **$73,00** | 🔴 **[ANOMALIE]** — inchangé, incohérent avec spot |
...
**Verdict Sentiment :** Neutre / Légèrement baissier — Aucune évolution depuis 17:00 UTC. Le biais call reste neutralisé. Aucun upgrade/downgrade. Aucune news. Le mouvement du 25 mai reste purement technique/momentum sans support informationnel.
...
| **Event-Driven** | Aucun événement corporate dans `events_2026-05-25.json`. | Aucun bonus/malus. |
...
> **Révision** : Niveaux inchangés vs snapshot 17:00 UTC (données identiques). Le SL expose à un drawdown de –18,3 % en 1–2 séances compte tenu du Beta 2,31 et de l'ATR $12,41.
...
| **Expiration options** | 2026-05-29 | **4 jours** | Max Pain $73,00 [ANOMALIE DATA] — à ignorer en l'état |

**Prochain catalyseur majeur** : Aucun avant earnings (août). L'expiration options du 29 mai approche dans 4 jours.
...
Le snapshot 21:00 UTC du 25 mai 2026 confirme la **stabilité totale** des données vs le snapshot 17:00 UTC. Le marché US était fermé pour le Memorial Day — aucune nouvelle cotation, aucune news, aucun mouvement.

**Éléments clés vs analyse précédente (17:00 UTC)** :
1. **Cours stable à $135,76** — identique au snapshot 17:00 UTC.
...
6. **🔴 [ANOMALIE DATA]** Max Pain $73,00 persistante — probable erreur flux Yahoo.
...
**Recommandation** : Maintenir la posture **ÉVITER**. La stabilité des données renforce la confiance dans la thèse opérationnelle : aucun catalyseur fondamental, valorisation déconnectée, surchauffe technique extrême. Attendre :
...
*Rapport généré le 2026-05-25 — Données : `data/latest.json` (21:00 UTC), `data/recommandations_2026-05-25.json`, `data/upcoming_events_2026-05-25.json`, `data/events_2026-05-25.json`, `data/news_2026-05-25.json`, `data/social_sentiment_2026-05-25.json`, `data/geo_risk_latest.json` (2026-05-17), `data/sector_rotation_2026-05-25.json`, `data/fx_exposure_2026-05-25.json`, `data/quant_report_latest.json` (2026-05-17)*
