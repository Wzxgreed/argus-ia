...
**ATTENDRE** (2026-05-26 · snapshot 10:00 UTC) — Qualité opérationnelle classée **hors périmètre (3/6)** après injection FMP FY 2025-12-31 : bilan structurellement négatif (tangible asset value −$32.5B, current ratio −0.18), rentabilité GAAP inexistante (ROIC −12.8%, net margin ~0%), et dilution SBC à 5% des revenus. La valorisation reflète cette dégradation (EV/EBITDA 18.2x, P/S 1.45x) sans marge de sécurité. Le timing est bloqué par l'**anomalie de données critique** : **cours figé $83.46 sur 13 snapshots consécutifs couvrant 6 jours calendaires** (20/05 → 26/05) et **earnings Q1 2026 non résolu après 6 jours** (date initialement prévue 20/05). Le marché US a rouvert ce jour (post-Memorial Day) et le pipeline a fetché des cours frais pour 22/26 tickers, mais SQ reste le **seul ticker avec `source: fmp_fallback` et `change_pct: null`**. Le quality gate marque désormais SQ comme **excluded** (CRITICAL stale price), bien qu'un bug systémique affecte également l'ensemble de la watchlist. Le fichier `recommandations_2026-05-26.json` affiche un RSI placeholder 50 mais le bloc `technical` est vide. Le secteur Technology bénéficie d'une rotation favorable (XLK top3), mais cela ne compense pas la fragilité fondamentale ni l'anomalie de données. Le pipeline du 26/05 a généré un statut **partial** (phases C/D failed, validate + detect_major_events failed, accounting skipped). Tout positionnement avant résolution du stale price et intégration des résultats Q1 est déconseillé. Priorité opérationnelle : vérifier cours live via broker, date réelle de publication des résultats, et forcer un re-fetch isolé de SQ.
...
|------|---------|------|------------|
| 2026-05-26 | [SQ_2026-05-26_update.md](SQ_2026-05-26_update.md) | Mise à jour snapshot **10:00 UTC** post-Memorial Day | **ATTENDRE** — Qualité 3/6, earnings **non résolu après 6 jours**, **stale price 13 snapshots / 6 jours calendaires (quality gate excluded officiellement)**, marché rouvert mais SQ reste seul ticker `fmp_fallback`, pipeline partial, zero changement vs 25/05 21:00 UTC |
...
- 🔴 **Data Pipeline Alert** — Earnings Q1 2026 non résolu après 6 jours calendaires (date initiale 20/05). `upcoming_events_2026-05-26.json` affiche `days_until: 0` avec date 26/05, mais champ details vide (placeholder FMP générique).
