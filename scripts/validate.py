...
SCHEMA_PATH = DATA_DIR / "latest.schema.json"


def load_schema() -> dict | None:
    """Charge le JSON Schema pour les snapshots."""
...
def validate_schema(snapshot: dict) -> list:
    """Valide le snapshot contre le JSON Schema. Retourne une liste d'issues."""
    schema = load_schema()
    if schema is None:
        return ["[WARNING] JSON Schema non trouvé — validation structurelle ignorée."]
    ...
    try:
        ...
        issues.append(f"[ERROR] JSON Schema violation at {path}: {e.message}")
    except Exception as e:
        issues.append(f"[WARNING] Erreur lors de la validation schema: {e}")

    if not issues:
        issues.append("[INFO] JSON Schema validation passed.")
...
def validate_snapshot(snapshot: dict) -> list:
    ...
    for ticker, data in prices.items():
        ...
        # Variation < 50% (split ou flash crash)
